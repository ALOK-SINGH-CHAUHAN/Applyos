...
// Helpers
// ---------------------------------------------------------------------------
function bundleIIFE(entryPoint, globalName) {
    const tmpFile = path.join(outDir, `_tmp_${Date.now()}_${Math.random().toString(36).slice(2)}.js`);
    try {
        esbuild.buildSync({
...
async function extractFunctionSources(entryPoint) {
    const tmpFile = path.join(outDir, `_tmp_${Date.now()}_${Math.random().toString(36).slice(2)}.mjs`);
    try {
        esbuild.buildSync({
            entryPoints: [entryPoint],
            bundle: true,
...
// Builders
// ---------------------------------------------------------------------------
async function buildIIFEString(config) {
...
async function buildBootstrapModule(config) {
    const entryPoint = path.join(domDir, config.entry);
    ...
    const sources = Object.fromEntries(functions);
    const globalRefs = Object.fromEntries(functions.map(([name]) => [
        name,
...
async function buildSourcesOnly(config) {
    const entryPoint = path.join(domDir, config.entry);
    const functions = await extractFunctionSources(entryPoint);
    const sources = Object.fromEntries(functions);
    const banner = `/*\n * AUTO-GENERATED FILE. DO NOT EDIT.\n * Run \`pnpm run build-dom-scripts\` to regenerate.\n */`;
...
async function main() {
    ...
    await Promise.all(scripts.map((config) => {
        switch (config.type) {
            case "iife-string":
