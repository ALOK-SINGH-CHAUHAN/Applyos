...
import { ensureParentDir, parseListFlag, splitArgs, collectFiles, toSafeName, writeCtrfFromJunit, } from "./test-utils.js";
import { createRequireFromCaller, getRepoRootDir, } from "../lib/v3/runtimePaths.js";
...
const hasConfigArg = (argsList) => argsList.some((arg, i) => {
    if (arg.startsWith("--config="))
        return true;
    return arg === "--config" && Boolean(argsList[i + 1]);
});
const stripReporterArgs = (argsList) => {
    const filtered = [];
    let removed = false;
...
const toPlaywrightPath = (testPath) => {
    const abs = resolveRepoRelative(testPath);
    const rel = path.relative(testsDir, abs).replaceAll("\\", "/");
...
if (!fs.existsSync(testsDir)) {
    console.error("Missing packages/core/dist/esm/tests/integration. Run pnpm run build:esm first.");
    process.exit(1);
}
const { filtered: extraArgs, removed: removedReporterOverride } = stripReporterArgs(extra);
if (removedReporterOverride) {
    console.warn("Ignoring Playwright --reporter override to preserve console + JUnit output.");
}
...
const coverageDir = resolveRepoRelative(process.env.NODE_V8_COVERAGE ??
    (relTestName
