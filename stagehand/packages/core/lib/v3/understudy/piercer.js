...
export async function installV3PiercerIntoSession(session) {
    ...
    try {
        await session.send("Page.addScriptToEvaluateOnNewDocument", { source: v3ScriptContent, runImmediately: true });
    }
    catch (e) {
...
export function tapPiercerConsole(session, label) {
