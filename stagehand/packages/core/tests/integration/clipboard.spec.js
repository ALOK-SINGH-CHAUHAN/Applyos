...
async function setupTextarea(page, options) {
...
async function textareaValue(page, id) {
    return await page.evaluate((selector) => document.querySelector(selector).value, `#${id}`);
}
async function selectTextareaContents(page, id) {
...
async function setupCopyButton(page, options) {
    await page.goto(TEST_URL, {
        waitUntil: "domcontentloaded",
        timeoutMs: 15000,
    });
