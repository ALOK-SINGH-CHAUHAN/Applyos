...
describe("tryScopedSnapshot", () => {
    ...
    it("returns scoped snapshot when focus selector resolves via CSS hops", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".btn" }, context, true, new Map([[session.id, sessionIndex]]), new Map());
    ...
    it("returns null and logs fallback when scope is not applied", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".btn" }, context, false, new Map([[session.id, sessionIndex]]), new Map());
        expect(result).toBeNull();
        expect(loggerSpy).toHaveBeenCalled();
    });
    it("returns null immediately when no focus selector is provided", async () => {
        const result = await tryScopedSnapshot(makePage(new MockCDPSession({})), {}, context, true, new Map(), new Map());
        expect(result).toBeNull();
    });
    it("supports XPath focus resolution branch", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: "xpath=//div" }, context, true, new Map([[session.id, sessionIndex]]), new Map());
        expect(result).not.toBeNull();
    ...
    it("logs and returns null when resolver throws", async () => {
        ...
        const loggerSpy = vi.spyOn(logger, "v3Logger").mockImplementation(() => { });
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".bad" }, context, true, new Map([[session.id, sessionIndex]]), new Map());
        expect(result).toBeNull();
        expect(loggerSpy).toHaveBeenCalled();
    });
    it("filters ignored xpath entries in the scoped snapshot", async () => {
        ...
        const result = await tryScopedSnapshot(makePage(session), { focusSelector: ".btn" }, context, true, new Map([[session.id, scopedIndex]]), new Map([["frame-2", [{ start: 0, end: 2 }]]]));
        expect(result?.combinedXpathMap["1-10"]).toBeUndefined();
        expect(result?.combinedXpathMap["1-11"]).toBe("/html/body/iframe[1]/div[2]");
    });
});
