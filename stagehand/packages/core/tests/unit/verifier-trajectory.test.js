...
import { loadTrajectoryFromDisk, nextResultFilename, normalizeRubric, writeTrajectoryDir, } from "../../lib/v3/verifier/trajectory.js";
describe("verifier trajectory utilities", () => {
    it("normalizes serialized empty earned points out of public rubrics", () => {
        expect(normalizeRubric({
            items: [
                {
                    criterion: "Criterion",
    ...
    it("loads trajectory screenshots and image modalities from disk", async () => {
        ...
        await writeFile(path.join(dir, "screenshots", "agent", "1.png"), agentImage);
        await writeFile(path.join(dir, "trajectory.json"), JSON.stringify({
    ...
    it("redacts inline screenshot payloads when writing trajectories", async () => {
        const dir = await mkdtemp(path.join(tmpdir(), "stagehand-verifier-"));
        const inlineScreenshot = Buffer.from("inline screenshot").toString("base64");
        ...
        expect(trajectory.steps[0].agentEvidence.modalities[0].content.output
            .screenshotBase64).toBe("[redacted inline image payload]");
        expect(trajectory.steps[0].toolOutput.result.output.screenshotBase64).toBe("[redacted inline image payload]");
        expect(trajectory.finalObservation.screenshotPath).toBe("screenshots/probe/final.png");
        await expect(readFile(path.join(dir, "screenshots", "probe", "final.png"))).resolves.toEqual(Buffer.from("final screenshot"));
    });
    it("rejects screenshot paths outside the trajectory directory", async () => {
        ...
        await expect(loadTrajectoryFromDisk(dir)).rejects.toThrow("escapes trajectory directory");
    });
    it("sanitizes result filename labels", () => {
        expect(nextResultFilename("rescore / task:one?")).toBe("result_rescore___task_one_.json");
    });
});
