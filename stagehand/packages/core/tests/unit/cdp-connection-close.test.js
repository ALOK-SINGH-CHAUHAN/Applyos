...
async function sendCdpEvent(serverSocket, message) {
...
async function waitForSession(conn, sessionId) {
    const session = await raceTimeout(new Promise((resolve) => {
        const check = () => {
...
describe("CdpConnection", () => {
    ...
    describe("session event listener cleanup", () => {
        it("removes session-scoped event handlers when a target detaches", async () => {
            ...
            expect(eventHandlers.has("session-a:Network.requestWillBeSent")).toBe(true);
            ...
            expect(eventHandlers.has("session-a:Network.requestWillBeSent")).toBe(false);
            expect(eventHandlers.has("session-b:Fetch.requestPaused")).toBe(true);
        ...
        it("rejects session dispatch waiters when a target is destroyed", async () => {
            ...
            const pending = pair.conn.waitForSessionDispatch("session-a", "Fetch.enable");
            const resultPromise = pending
                .then(() => "resolved")
                .catch(() => "rejected");
            await sendCdpEvent(pair.serverSocket, {
