...
beforeEach(async () => {
    ...
    launchMock = chromeLauncher.launch;
    launchMock.mockClear();
});
afterEach(() => {
    vi.unstubAllGlobals();
});
async function getLaunchArgs(opts) {
...
describe("launchLocalChrome ignoreDefaultArgs", () => {
    ...
    it("selectively removes only the listed flag", async () => {
        ...
        expect(args.chromeFlags).toContain("--disable-component-extensions-with-background-pages");
    ...
    it("keeps Stagehand's own flags when selectively removing defaults", async () => {
        ...
        expect(args.chromeFlags).toContain("--remote-allow-origins=*");
        expect(args.chromeFlags).toContain("--no-first-run");
        expect(args.chromeFlags).toContain("--enable-features=WebMCPTesting,DevToolsWebMCPSupport");
    ...
    it("allows users to disable the Stagehand --site-per-process default", async () => {
        ...
        expect(args.chromeFlags).toContain("--disable-features=site-per-process,IsolateOrigins");
