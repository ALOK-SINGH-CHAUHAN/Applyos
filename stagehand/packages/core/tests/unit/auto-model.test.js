...
describe("per-call model 'auto' local resolution", () => {
    it("throws when resolving 'auto' without an API client", () => {
        ...
        expect(() => privateStagehand.resolveLlmClient("auto")).toThrow(StagehandInvalidArgumentError);
    });
    it("reuses the default client when resolving 'auto' with an API client", () => {
        ...
        expect(privateStagehand.resolveLlmClient("auto")).toBe(privateStagehand.llmClient);
    });
});
describe("agent cache with model 'auto'", () => {
    function createAgentCache(baseModelName) {
        return new AgentCache({
            storage: { enabled: true },
            logger: vi.fn(),
            getActHandler: () => null,
            getContext: () => null,
            getDefaultLlmClient: () => undefined,
    ...
    it("skips local replay for 'auto' sessions (no local client for self-heal)", () => {
        expect(createAgentCache("auto").shouldAttemptCache("do the thing")).toBe(false);
    });
    it("still replays for concrete-model sessions", () => {
        expect(createAgentCache("openai/gpt-4.1-mini").shouldAttemptCache("do the thing")).toBe(true);
    });
});
