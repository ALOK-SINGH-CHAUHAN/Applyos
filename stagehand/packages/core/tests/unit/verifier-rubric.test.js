...
describe("RubricVerifier", () => {
    ...
    afterEach(() => {
        ...
        restoreEnv("VERIFIER_RUBRIC_REQUIRE_TASK_SPAN", previousEnv.requireTaskSpan);
    });
    it("retries rubric generation and filters criteria outside the task span", async () => {
                ...
                items: [
                    {
                        ...
                        description: "Full credit for identifying the most recent relevant paper.",
                        max_points: 4,
                        justification: "",
                        earned_points: "",
                    },
        ...
        const verifier = new RubricVerifier({
            ...
            getRubricGenClient: () => ({ createChatCompletion }),
            logger: vi.fn(),
        });
        const rubric = await verifier.generateRubric({
            id: "arxiv",
            instruction: "Search arXiv for the most recent paper on retrieval-augmented generation.",
        });
    ...
    it("supports outcome-only verification without generating a rubric", async () => {
        ...
        const verifier = new RubricVerifier({
            getClient: () => ({ createChatCompletion }),
        ...
        const result = await verifier.verify(makeTrajectory(taskSpec, Buffer.from("screenshot")));
    ...
    it("always attaches the final probe screenshot and ariaTree to outcome verification", async () => {
        ...
        process.env.VERIFIER_APPROACH = "outcome-only";
        const seenPrompt = vi.fn();
        const seenImageBytesLengths = vi.fn();
        ...
        const finalScreenshot = Buffer.from("final-page-with-$305-trade-in-value-bytes");
        const finalAria = "RootWebArea: Apple Trade In\n  heading: Get $305 trade-in credit toward a new iPhone.\n  StaticText: Trade-in values are estimates";
        const trajectory = {
            task: taskSpec,
            steps: [
                {
...
function makeTrajectory(task, screenshot) {
    return {
        task,
        steps: [
            {
