...
describe("StagehandAPIClient default model config", () => {
    ...
    it("sends constructor Vertex model config on navigate bootstrap, act, observe, extract, and agent execute requests", async () => {
        ...
        expect(executeMock).toHaveBeenNthCalledWith(1, expect.objectContaining({
            method: "navigate",
        ...
        expect(executeMock).toHaveBeenNthCalledWith(2, expect.objectContaining({
            method: "act",
            args: expect.objectContaining({
                options: {
                    model: expect.objectContaining({
        ...
        expect(executeMock).toHaveBeenNthCalledWith(3, expect.objectContaining({
            method: "observe",
            args: expect.objectContaining({
        ...
        expect(executeMock).toHaveBeenNthCalledWith(4, expect.objectContaining({
            method: "extract",
            args: expect.objectContaining({
                options: expect.objectContaining({
                    model: expect.objectContaining({
        ...
        expect(executeMock).toHaveBeenNthCalledWith(5, expect.objectContaining({
            method: "agentExecute",
            args: expect.objectContaining({
    ...
    it("does not serialize AZURE_API_KEY when constructor Azure Entra auth provides provider credentials", async () => {
        ...
        const model = executeMock.mock.calls[0][0].args.options.model;
    ...
    it("does not load AZURE_API_KEY into local constructor config when Azure Entra auth provides provider credentials", () => {
        ...
        const modelClientOptions = stagehand.modelClientOptions;
        expect(modelClientOptions).toEqual(expect.objectContaining({
            auth: expect.objectContaining({
                type: "azureEntraId",
                token: "test-entra-token",
            }),
