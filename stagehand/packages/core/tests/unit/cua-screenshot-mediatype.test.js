...
describe("CUA clients thread screenshot mediaType through captureScreenshot", () => {
    ...
    let microsoft;
    beforeEach(() => {
        google = new GoogleCUAClient("google", "gemini-2.5-computer-use-preview-10-2025", undefined, {
            apiKey: "test",
        });
        anthropic = new AnthropicCUAClient("anthropic", "claude-sonnet-4-5-20250929", undefined, {
            apiKey: "test",
        });
    ...
    it("OpenAI/Microsoft build the data URL with the provider's mediaType", async () => {
        ...
        expect(await microsoft.captureScreenshot()).toBe("data:image/jpeg;base64,jpeg-bytes");
    });
