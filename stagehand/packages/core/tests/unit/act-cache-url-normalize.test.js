...
describe("normalizeUrlForCacheKey", () => {
    ...
    it("sorts query parameters alphabetically", () => {
        expect(normalizeUrlForCacheKey("https://example.com/?b=2&a=1&c=3")).toBe("https://example.com/?a=1&b=2&c=3");
    });
    it("produces identical output for URLs that differ only in parameter order", () => {
        const a = normalizeUrlForCacheKey("https://shop.example.com/cart?utm_source=email&id=42");
        const b = normalizeUrlForCacheKey("https://shop.example.com/cart?id=42&utm_source=email");
        expect(a).toBe(b);
    });
    it("preserves duplicate keys in their original order", () => {
        ...
        expect(normalizeUrlForCacheKey("https://example.com/?a=1&a=2")).toBe("https://example.com/?a=1&a=2");
        expect(normalizeUrlForCacheKey("https://example.com/?a=2&a=1")).toBe("https://example.com/?a=2&a=1");
    });
    it("preserves the URL fragment", () => {
        // Fragments often indicate distinct views in SPAs, so we deliberately
        // do not strip them.
        expect(normalizeUrlForCacheKey("https://docs.example.com/guide#install")).toBe("https://docs.example.com/guide#install");
    });
    it("preserves path and origin", () => {
        expect(normalizeUrlForCacheKey("https://sub.example.com:8443/a/b/c?z=1&a=2")).toBe("https://sub.example.com:8443/a/b/c?a=2&z=1");
    });
