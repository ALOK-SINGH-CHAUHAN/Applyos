...
import { getAISDKLanguageModel, LLMProvider, toAISDKClientOptions, } from "../../lib/v3/llm/LLMProvider.js";
...
describe("LLMProvider", () => {
    it("allows Vertex models without experimental mode", () => {
        ...
        expect(() => provider.getClient("vertex/gemini-2.5-flash", {
            auth: {
        ...
        }, { experimental: false, disableAPI: false })).not.toThrow();
    });
    it("adapts canonical Vertex auth into AI SDK googleAuthOptions", () => {
        expect(toAISDKClientOptions("vertex", {
            auth: {
                type: "googleServiceAccount",
                credentials: {
                    client_email: "stagehand@example.iam.gserviceaccount.com",
    ...
    it("allows Azure models without experimental mode", () => {
        const provider = new LLMProvider(() => { });
        expect(() => provider.getClient("azure/gpt-4.1-mini", {
            auth: {
                type: "azureEntraId",
    ...
    it("adapts canonical Azure Entra auth into an AI SDK tokenProvider", async () => {
        ...
        await expect(tokenProvider()).resolves.toBe("test-entra-token");
    });
