...
class FakeCuaClient {
    ...
    setViewport = vi.fn();
    setCurrentUrl = vi.fn();
    setScreenshotProvider = vi.fn((provider) => {
...
describe("v3 cua handler screenshot behavior", () => {
    ...
    it("does not take per-action screenshots when a batch of actions runs", async () => {
        ...
        vi.spyOn(handler, "executeAction").mockResolvedValue({ success: true });
        await handler.execute({
    ...
    it("still returns provider screenshots when screenshot evidence callbacks fail", async () => {
        ...
        const onEvidence = vi.fn(async (event) => {
        ...
        expect(onEvidence).toHaveBeenCalledWith(expect.objectContaining({ type: "screenshot" }));
        expect(logs.some((line) => line.message.includes("onEvidence callback failed for screenshot"))).toBe(true);
    });
    it("records a failed action as step_finished {ok:false} and rethrows the original error", async () => {
        ...
        vi.spyOn(handler, "executeAction").mockRejectedValue(new Error("click failed"));
