...
import { StagehandInvalidArgumentError, StagehandUnsupportedBrowserFeatureError, } from "../../lib/v3/types/public/sdkErrors.js";
...
describe("Page WebMCP", () => {
    ...
    it("throws a browser feature error when WebMCP.enable is unsupported while listing", async () => {
        ...
        const page = makePage(session);
        await expect(page.listWebMCPTools({ timeoutMs: 1 })).rejects.toThrow(StagehandUnsupportedBrowserFeatureError);
        await expect(page.listWebMCPTools({ timeoutMs: 1 })).rejects.toThrow("Chrome/Chromium newer than version 149");
        await expect(page.listWebMCPTools({ timeoutMs: 1 })).rejects.toThrow("Stagehand's local Chrome launcher enables WebMCP flags by default");
    });
    it("invokes a tool with an explicit frameId and resolves the response event", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("echo", { text: "hello" }, { frameId: "frame-1", timeoutMs: 100 });
    ...
    it("throws a browser feature error when invokeTool is unsupported", async () => {
        ...
        const page = makePage(session);
        await expect(page.invokeWebMCPTool("echo", {}, { frameId: "frame-1", timeoutMs: 1 })).rejects.toThrow(StagehandUnsupportedBrowserFeatureError);
        await expect(page.invokeWebMCPTool("echo", {}, { frameId: "frame-1", timeoutMs: 1 })).rejects.toThrow("Chrome/Chromium newer than version 149");
    });
    it("rethrows invokeTool argument or state errors without classifying them as unsupported browser features", async () => {
        const invocationError = new Error("No tool named echo is registered in frame-1");
        const session = new MockCDPSession({
            "WebMCP.invokeTool": () => {
    ...
    it("uses listWebMCPTools to resolve a unique frameId when omitted", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("echo", {}, { timeoutMs: 100 });
        session.emit("WebMCP.toolResponded", {
            invocationId: "invocation-1",
            status: "Completed",
        });
    ...
    it("throws when a tool name is ambiguous across frames", async () => {
        ...
        const page = makePage(session);
        await expect(page.invokeWebMCPTool("echo", {}, { timeoutMs: 1 })).rejects.toThrow(StagehandInvalidArgumentError);
        await expect(page.invokeWebMCPTool("echo", {}, { timeoutMs: 1 })).rejects.toThrow("multiple frames");
    });
    it("rejects invocation.result when the response times out", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("slow", {}, { frameId: "frame-1", timeoutMs: 1 });
        await expect(invocation.result).rejects.toThrow('Timed out waiting for WebMCP tool "slow"');
    });
    it("buffers a response event that arrives before invokeTool returns", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("early", {}, { frameId: "frame-1", timeoutMs: 100 });
        await expect(invocation.result).resolves.toEqual({
            invocationId: "invocation-1",
            status: "Completed",
            output: "early",
    ...
    it("does not buffer late responses after an invocation times out", async () => {
        ...
        const page = makePage(session);
        const timedOutInvocation = await page.invokeWebMCPTool("slow", {}, { frameId: "frame-1", timeoutMs: 1 });
        await expect(timedOutInvocation.result).rejects.toThrow('Timed out waiting for WebMCP tool "slow"');
        session.emit("WebMCP.toolResponded", {
            invocationId: "invocation-1",
            status: "Completed",
            output: "late",
        });
        const currentInvocation = await page.invokeWebMCPTool("current", {}, { frameId: "frame-1", timeoutMs: 100 });
    ...
    it("sends cancelInvocation without removing the pending result listener", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("cancelable", {}, { frameId: "frame-1", timeoutMs: 100 });
    ...
    it("removes persistent WebMCP response listener and rejects pending invocations on close", async () => {
        ...
        const page = makePage(session);
        const invocation = await page.invokeWebMCPTool("pending", {}, { frameId: "frame-1", timeoutMs: 100 });
        expect(session.listenerCount("WebMCP.toolResponded")).toBe(1);
        await page.close();
        expect(session.listenerCount("WebMCP.toolResponded")).toBe(0);
        await expect(invocation.result).rejects.toThrow('WebMCP invocation "invocation-1" was disposed');
    });
});
