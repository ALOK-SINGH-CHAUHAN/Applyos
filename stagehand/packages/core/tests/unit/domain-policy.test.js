...
import { getDomainPolicyDecision, normalizeDomainPolicy, } from "../../lib/v3/understudy/domainPolicy.js";
...
describe("domain policy helpers", () => {
    ...
    it("matches exact and wildcard domains without matching unrelated suffixes", () => {
        ...
        expect(getDomainPolicyDecision("https://ads.example.com/script.js", policy)).toMatchObject({ action: "block" });
        expect(getDomainPolicyDecision("https://a.tracking.example.com/pixel.gif", policy)).toMatchObject({ action: "block" });
        expect(getDomainPolicyDecision("https://deep.a.tracking.example.com/pixel.gif", policy)).toMatchObject({ action: "block" });
        expect(getDomainPolicyDecision("https://ads.example.com./script.js", policy)).toMatchObject({ action: "block" });
        expect(getDomainPolicyDecision("https://a.tracking.example.com./pixel.gif", policy)).toMatchObject({ action: "block" });
        expect(getDomainPolicyDecision("https://tracking.example.com/", policy)).toEqual({ action: "continue" });
        expect(getDomainPolicyDecision("https://badtracking.example.com/", policy)).toEqual({ action: "continue" });
        expect(getDomainPolicyDecision("https://ads.example.com.evil.test/", policy)).toEqual({ action: "continue" });
    });
    ...
    it("allows matching exact and wildcard allowed domains", () => {
        ...
        expect(getDomainPolicyDecision("https://deep.app.example.com/", policy)).toEqual({
            action: "continue",
        });
    ...
    it("lets blocked domains win over allowed domains", () => {
        ...
        expect(getDomainPolicyDecision("https://ads.example.com/script.js", policy)).toEqual({
            action: "block",
    ...
    it("rejects omitted policy and non-array domain fields", () => {
        expect(() => normalizeDomainPolicy(undefined)).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({
            allowedDomains: "example.com",
        ...
        expect(() => normalizeDomainPolicy({
            blockedDomains: new Set(["ads.example.com"]),
        })).toThrow(StagehandInvalidArgumentError);
    });
    it("rejects invalid blocked domain patterns", () => {
        expect(() => normalizeDomainPolicy({ blockedDomains: ["https://example.com"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({ blockedDomains: ["*example.com"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({ blockedDomains: ["example"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({
            blockedDomains: [123],
        })).toThrow(StagehandInvalidArgumentError);
    });
    it("rejects invalid allowed domain patterns", () => {
        expect(() => normalizeDomainPolicy({ allowedDomains: ["https://example.com"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({ allowedDomains: ["*example.com"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({ allowedDomains: ["example"] })).toThrow(StagehandInvalidArgumentError);
        expect(() => normalizeDomainPolicy({
            allowedDomains: [123],
        })).toThrow(StagehandInvalidArgumentError);
    });
