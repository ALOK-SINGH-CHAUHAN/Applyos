...
describe("collectPerFrameMaps", () => {
    it("builds per-frame xpath/tag maps and outlines from a shared session index", async () => {
        ...
        const result = await capture.collectPerFrameMaps(page, context, sessionToIndex, { experimental: true }, true, context.frames, new Map());
    ...
    it("builds a missing session index on demand and memoizes it", async () => {
        ...
        const result = await capture.collectPerFrameMaps(page, context, sessionToIndex, undefined, false, context.frames, new Map());
    ...
    it("skips frames that are not listed in the frameIds argument", async () => {
        ...
        const result = await capture.collectPerFrameMaps(page, context, sessionToIndex, undefined, true, ["frame-1"], new Map());
...
describe("captureHybridSnapshot", () => {
    ...
    it("filters ignored nodes out of the merged snapshot artifacts", async () => {
        const session = new MockCDPSession({
            ...
            "DOM.describeNode": async (params) => ({
                node: {
                    backendNodeId: params?.objectId === "ignored-object-a"
                        ? 201
        ...
        vi.spyOn(a11yTree, "a11yForFrame").mockImplementation(async (_sess, frameId, opts) => ({
            outline: opts.isIgnoredBackendNode?.(201) && opts.isIgnoredBackendNode?.(202)
                ? `outline-${frameId}-filtered`
                : `outline-${frameId}`,
            urlMap: frameId === "frame-2" &&
        ...
        expect(snapshot.perFrame?.find((frame) => frame.frameId === "frame-2")?.outline).toContain("filtered");
    });
    it("excludes child frame subtrees when an ignored node is an iframe host", async () => {
        ...
        vi.spyOn(domTree, "buildSessionDomIndex").mockResolvedValue(makeSessionIndex());
        vi.spyOn(focusSelectors, "resolveCssFocusFrameAndTail").mockResolvedValue({
            targetFrameId: "frame-1",
        ...
        vi.spyOn(a11yTree, "a11yForFrame").mockImplementation(async (_sess, frameId, opts) => ({
            outline: frameId === "frame-2" && opts.isIgnoredBackendNode?.(200)
                ? ""
                : `outline-${frameId}`,
            urlMap: frameId === "frame-2" && opts.isIgnoredBackendNode?.(200)
                ? {}
                : { [`url-${frameId}`]: `https://${frameId}.test` },
            scopeApplied: false,
        }));
