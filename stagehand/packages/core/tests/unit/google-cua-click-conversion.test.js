...
const convert = GoogleCUAClient.prototype.convertFunctionCallToAction;
function createGoogleClient() {
    return new GoogleCUAClient("google", "google/gemini-3.5-flash", "test instructions", { apiKey: "test" });
}
function run(name, args) {
    return convert.call(createGoogleClient(), { name, args });
}
...
describe("GoogleCUAClient gemini-3.x click-family conversion", () => {
    it("maps click to a left click (backcompat with 2.5 click_at)", () => {
        ...
        expect(Number.isFinite(action?.y)).toBe(true);
    });
    ...
    it("returns null when click-family coordinates are NaN or Infinity", () => {
        ...
        expect(run("drag_and_drop", { start_x: NaN, start_y: 1, end_x: 2, end_y: 3 })).toBeNull();
    });
