...
async function example() {
    ...
    await new Promise((resolve) => setTimeout(resolve, 3000));
    [observation] = await stagehand.observe("find the '1+' button in the 'beds' section");
    await stagehand.act(observation);
    await new Promise((resolve) => setTimeout(resolve, 3000));
    [observation] = await stagehand.observe("find the 'apartments' button in the 'home type' section");
    await stagehand.act(observation);
    await new Promise((resolve) => setTimeout(resolve, 3000));
    [observation] = await stagehand.observe("find the pet policy dropdown to click on.");
    await stagehand.act(observation);
    await new Promise((resolve) => setTimeout(resolve, 3000));
    [observation] = await stagehand.observe("find the 'Dog Friendly' option to click on");
    await stagehand.act(observation);
    await new Promise((resolve) => setTimeout(resolve, 3000));
    ...
    if (currentUrl.includes("https://www.apartments.com/apartments/san-francisco-ca/min-1-bedrooms-pet-friendly-dog/")) {
