...
function findValidatedAction(observed, placeholder, keywords) {
    const matches = observed.filter((action) => {
        const description = action.description.toLowerCase();
        return (action.arguments?.includes(placeholder) === true &&
            keywords.some((keyword) => description.includes(keyword)));
    });
    if (matches.length !== 1) {
        throw new Error(`Expected exactly one safe action for ${placeholder}, found ${matches.length}`);
    }
    return matches[0];
}
async function observeVariablesLogin() {
    ...
    try {
        ...
        console.log(`${chalk.green("Observe:")} Placeholder actions found:\n${observed
            .map((action) => `${chalk.yellow(action.description)} -> ${chalk.blue(action.arguments?.join(", ") || "no arguments")}`)
            .join("\n")}`);
        ...
        console.log(`\n${chalk.green("Validated:")} Safe actions to execute:\n${[
            ...
            .map((action) => `${chalk.yellow(action.description)} -> ${chalk.blue(action.arguments?.[0] || "no value")}`)
            .join("\n")}`);
