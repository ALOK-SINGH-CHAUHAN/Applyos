...
async function example(stagehand) {
    ...
    const [page1Extraction, page2Extraction] = await Promise.all([
        stagehand.extract("extract the names of the four stagehand primitives", z.array(z.string()), { page: pwPage1 }),
        stagehand.extract("extract the list of session dashboard features", z.array(z.string()), { page: pwPage2 }),
    ]);
