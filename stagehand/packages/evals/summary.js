...
export const generateSummary = async (results, experimentName, experimentUrl, scores) => {
    const getTaskBasename = (taskName) => {
        if (!taskName.includes("/"))
            return taskName;
        const parts = taskName.split("/");
        return parts[parts.length - 1] ?? taskName;
    };
    const resolveCategories = (taskName) => {
