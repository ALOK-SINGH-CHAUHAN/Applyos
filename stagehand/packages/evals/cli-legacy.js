...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀`));
    ...
    console.log(chalk.green("  evals run benchmark:onlineMind2Web") +
...
function handleRun(args) {
    ...
    const isCoreTarget = target === "core" ||
    ...
    if (target) {
        if (target.startsWith("b:") || target.startsWith("benchmark:")) {
            ...
            if (!config.benchmarks[benchmarkName]) {
                ...
                console.log(chalk.dim(`Available benchmarks: ${Object.keys(config.benchmarks).join(", ")}`));
                process.exit(1);
            }
    ...
    buildChild.on("exit", (buildCode) => {
        ...
        const sourceEvalPath = path.resolve(moduleDir, "..", "..", "packages", "evals", "index.eval.ts");
        let child;
