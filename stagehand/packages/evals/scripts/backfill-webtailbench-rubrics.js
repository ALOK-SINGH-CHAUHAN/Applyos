...
const HF_URL = "https://huggingface.co/datasets/microsoft/WebTailBench/resolve/main/WebTailBench-v1-rubrics.tsv";
const REPO_ROOT = path.resolve(import.meta.dirname, "..", "..", "..");
const JSONL_PATH = path.join(REPO_ROOT, "packages", "evals", "datasets", "webtailbench", "WebTailBench_data.jsonl");
/**
...
async function main() {
    ...
    if (idIdx === -1 || rubricIdx === -1) {
        throw new Error(`unexpected TSV header: ${header.join(", ")} (need 'id' and 'precomputed_rubric')`);
    }
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (!cols[idIdx])
            continue;
        try {
            const parsed = JSON.parse(cols[rubricIdx]);
            rubricsById.set(cols[idIdx], parsed);
        }
        catch (e) {
            console.warn(`  ! row ${i} (id=${cols[idIdx]}) — invalid JSON in precomputed_rubric: ${e instanceof Error ? e.message : e}`);
        }
    }
    ...
    console.log(`  ✓ matched ${matched}/${inLines.length} rows; ${missing} unmatched (will fall back to generated rubrics)`);
