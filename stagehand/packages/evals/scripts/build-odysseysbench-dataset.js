...
async function main() {
    ...
    for (const task of tasks) {
        ...
        for (const k of rubricKeys) {
            ...
            if (typeof r.weight !== "number" ||
                ...
                r.weight <= 0 ||
                r.weight > 1) {
                throw new Error(`Task ${task.task_id} rubric ${k} has invalid weight ${r.weight}; expected a number in (0, 1]`);
            }
        }
    ...
    const byLevel = tasks.reduce((acc, t) => {
