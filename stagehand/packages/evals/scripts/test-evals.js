...
const readEvalSummary = (summaryPath) => {
...
const escapeXml = (value) => value
    ...
    .replaceAll('"', "&quot;")
    .replaceAll("'", "&apos;");
const writeEvalJunit = (summaryPath, outputPath, category) => {
    ...
    const cases = [];
    if (missingSummary) {
        cases.push(`    <testcase name="${escapeXml(`evals/${category} summary missing`)}" classname="${escapeXml(suiteName)}" time="0">`, `      <failure message="eval summary missing">Missing eval summary at ${escapeXml(summaryPath)}</failure>`, "    </testcase>");
    }
    else {
        for (const item of passed) {
            cases.push(`    <testcase name="${escapeXml(`evals/${item.eval} [${item.model}]`)}" classname="${escapeXml(suiteName)}" time="0" />`);
        }
        for (const item of failed) {
            cases.push(`    <testcase name="${escapeXml(`evals/${item.eval} [${item.model}]`)}" classname="${escapeXml(suiteName)}" time="0">`, `      <failure message="eval failed">${escapeXml(`categories=${(item.categories ?? []).join(",")}`)}</failure>`, "    </testcase>");
        }
    }
    const xml = [
...
const writeEvalCtrf = (summaryPath, outputPath, category) => {
    const timestamp = new Date().toISOString();
    const summary = readEvalSummary(summaryPath);
    if (summary) {
        const passed = summary.passed ?? [];
        const failed = summary.failed ?? [];
        const toTests = (arr, status) => arr.map((item) => ({
...
const inferRuntimeFromPath = (value) => {
...
const inferRuntimeFromExecution = () => inferRuntimeFromPath(getCurrentFilePath()) ??
    inferRuntimeFromPath(process.cwd());
const rawArgs = process.argv.slice(2).filter((arg) => arg !== "--");
const listRequested = rawArgs.includes("--list");
const stripCliArg = (values) => {
    const filtered = [];
    let cliPath = null;
...
if (listRequested) {
    const categories = (process.env.EVAL_CATEGORIES ??
        "observe,act,combination,extract,targeted_extract,regression,agent").split(",");
...
const runtime = (strippedCli.cliPath
...
if (args.includes("--help") || args.includes("-h") || args[0] === "help") {
    const result = spawnSync(process.execPath, [...(runtime === "source" ? ["--import", "tsx"] : []), cliPath, "--help"], {
        stdio: "inherit",
        cwd: repoRoot,
    });
...
const nodeOptions = [process.env.NODE_OPTIONS, baseNodeOptions]
    .filter(Boolean)
    .join(" ");
const coverageDir = resolveRepoRelative(process.env.NODE_V8_COVERAGE ?? `${repoRoot}/coverage/evals/${safeTarget}`);
...
const result = spawnSync(process.execPath, [...(runtime === "source" ? ["--import", "tsx"] : []), cliPath, ...cliArgs], {
    stdio: "inherit",
    env,
    cwd: repoRoot,
});
