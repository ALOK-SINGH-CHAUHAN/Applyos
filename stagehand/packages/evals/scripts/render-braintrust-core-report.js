...
import { benchCaseDiffs, collectExperimentMetrics, detectCompareMode, fetchManyExperimentData, findLeaderIndex, sharedMetricKeys, sharedBenchCaseKeys, sharedTaskNames, summarizeBenchAgentConfigs, } from "../lib/braintrust-report.js";
const DEFAULT_PROJECT = "stagehand-core-dev";
const DEFAULT_TITLE = "Experiment Comparison";
const PHASE_METRICS = ["startup_ms", "task_ms", "cleanup_ms"];
...
function parseExperimentSpec(raw) {
    ...
    const looksLikeId = (s) => /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s) ||
...
function parseArgs(argv) {
    ...
    const experiments = positional.map((raw, i) => {
        ...
        if (labelOverrides[i])
            spec.label = labelOverrides[i];
        if (projectMap) {
            if (projectMap.length !== positional.length) {
                throw new Error(`--project-map length (${projectMap.length}) must match experiment count (${positional.length})`);
            }
...
function buildSummary(rows, shared, sharedTimers) {
...
function buildPhaseBreakdown(rows) {
    const totals = rows.map((r) => PHASE_METRICS.reduce((s, k) => s + (r.taskMetrics[k]?.mean ?? 0), 0));
...
function buildTimerGrid(rows, keys) {
    const timerKeys = keys.filter((k) => !PHASE_METRICS.includes(k) &&
        k !== "total_ms");
...
function buildTaskTable(rows, taskNames) {
    ...
    perRow.sort((a, b) => {
        if (a.anyFail !== b.anyFail)
            return a.anyFail ? -1 : 1;
    ...
    const body = perRow
        ...
        const cellsHtml = r.cells
            ...
            const minT = Math.min(...r.cells
                .filter((c) => !!c && c.totalMs !== undefined)
                .map((c) => c.totalMs));
            const isFastest = cell.totalMs === minT &&
...
function buildBenchSummary(rows) {
    const leader = findLeaderIndex(rows);
...
function agentConfigCell(summary) {
    if (!summary)
        return `<td class="xcol muted">—</td>`;
    const duration = summary.meanDurationMs !== undefined
...
function buildAgentConfigTable(rows) {
    const summaries = rows.map((row) => new Map(summarizeBenchAgentConfigs(row.benchCases).map((summary) => [
        summary.key,
        summary,
    ])));
    const configKeys = [
...
function buildExperimentMetricsTable(rows) {
    ...
    const body = metrics
        ...
        const cells = metric.values
            .map((value) => `<td class="xcol"><code>${escapeHtml(formatMetricValue(value, metric.unit, metric.key))}</code></td>`)
            .join("");
        return `
        <tr>
          <th class="task">
...
function buildBenchDiffTable(rows, diffs, title, description) {
    if (diffs.length === 0) {
        return `
      <section class="card">
        <header class="card__head">
...
function buildBenchContent(rows) {
    ...
    ${buildBenchDiffTable(rows, differingCases, "Case differences", "Shared case keys where pass/fail outcomes differ between experiments.")}
    ${buildBenchDiffTable(rows, missingCases, "Missing cases", "Case keys that exist in at least one experiment but not all experiments.")}
  `;
}
function buildHtml(title, rows) {
    ...
    `;
    const analyzedCount = mode === "bench"
