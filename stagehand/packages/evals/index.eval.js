...
import { buildTrajectoryGroupSlug, generateRunToken, resolveUnambiguousModel, resolveTrajectoryRoot, writeExperimentLink, } from "./framework/trajectoryGroup.js";
import { exactMatch, errorMatch } from "./scoring.js";
import { tasksByName, tasksConfig, getModelList, getAgentModelEntries, validateEvalName, } from "./taskConfig.js";
import { Eval } from "braintrust";
import { EvalLogger } from "./logger.js";
import { StagehandEvalError, AgentProvider, loadApiKeyFromEnv, getAISDKLanguageModel, } from "@browserbasehq/stagehand";
 ...
 */
function resolveTaskModulePath(baseDir, taskName) {
...
const generateFilteredTestcases = () => {
    let taskNamesToRun;
    let effectiveCategory = filterByCategory; // Start with the command-line filter
    if (filterByEvalName) {
        ...
        if (taskCategories.length === 1 &&
            ...
            console.log(`Task ${filterByEvalName} is in ${taskCategories[0]} category, using agent models.`);
        }
    }
    else if (filterByCategory) {
        // If filtering by category, get all tasks in that category
        taskNamesToRun = Object.keys(tasksByName).filter((name) => tasksByName[name].categories.includes(filterByCategory));
    }
    else {
        // If no specific task or category filter, run tasks from default categories
        taskNamesToRun = Object.keys(tasksByName).filter((name) => DEFAULT_EVAL_CATEGORIES.some((category) => tasksByName[name].categories.includes(category)));
    }
    ...
    else if (isWebVoyagerTaskIncluded &&
        datasetFilter &&
    ...
    else if (isMind2WebTaskIncluded &&
        datasetFilter &&
    ...
    else if (isWebTailBenchTaskIncluded &&
        datasetFilter &&
    ...
    if (isOdysseysBenchTaskIncluded &&
    ...
    else if (isOdysseysBenchTaskIncluded &&
        datasetFilter &&
    ...
    const regularTestcases = modelEntries.flatMap((entry) => taskNamesToRun.map((testName) => ({
        input: {
            name: testName,
        ...
        tags: [
            ...
            testName,
            ...(tasksConfig.find((t) => t.name === testName)?.categories || []).map((x) => `category/${x}`),
        ],
        metadata: {
    ...
    if (filterByCategory) {
        allTestcases = allTestcases.filter((testcase) => tasksByName[testcase.name].categories.includes(filterByCategory));
    }
    // If running in BROWSERBASE environment, exclude tasks that are not applicable.
    if (env === "BROWSERBASE") {
        allTestcases = allTestcases.filter((testcase) => !["peeler_simple", "stock_x"].includes(testcase.name));
    }
    console.log("Final test cases to run:", allTestcases
...
(async () => {
    ...
    try {
        ...
        if (runModel)
            process.env.EVAL_TRAJECTORY_MODEL = runModel;
        else
        ...
        const evalResult = await Eval(braintrustProjectName, {
            ...
            task: async (input) => {
                ...
                try {
                    ...
                    if (!taskModulePath) {
                        throw new StagehandEvalError(`Failed to find task module for ${input.name}. ` +
                    ...
                    if (typeof taskFunction !== "function" && taskModule.default) {
                        ...
                        if (defaultExport.__taskDefinition === true &&
                            typeof defaultExport.fn === "function") {
                            taskFunction = defaultExport.fn;
                        }
                    }
                    if (typeof taskFunction !== "function") {
                        throw new StagehandEvalError(`No Eval function found for task name: ${taskName} in module ${input.name}`);
                    }
                    // Execute the task
                    ...
                    if (USE_API) {
                        ...
                        if (!apiKey) {
                            throw new StagehandEvalError(`USE_API=true but no API key found for provider “${provider}”.`);
                        }
        ...
        const summaryResults = evalResult.results.map((result) => {
            const output = typeof result.output === "boolean"
