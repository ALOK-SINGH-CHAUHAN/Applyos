...
export const buildOdysseysBenchTestcases = (models) => {
    const odysseysbenchFilePath = getPackageRootDir() + "/datasets/odysseysbench/OdysseysBench_data.jsonl";
    ...
    const maxCases = parsePositiveIntEnv(process.env.EVAL_MAX_K) ??
    ...
    function isOdysseysBenchRow(parsed) {
    ...
    if (explicitIds && explicitIds.length > 0) {
        ...
        const levelFilter = process.env.EVAL_ODYSSEYSBENCH_LEVEL
            ? new Set(process.env.EVAL_ODYSSEYSBENCH_LEVEL.split(",")
