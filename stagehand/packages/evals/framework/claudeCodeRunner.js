...
export function normalizeClaudeCodeModel(model) {
    return model.includes("/") ? model.slice(model.indexOf("/") + 1) : model;
}
export function buildClaudeCodePrompt(plan, toolInstructions) {
    return [
...
function tryParseClaudeCodeJson(candidate) {
    try {
...
export function isClaudeCodeMaxTurnsError(value) {
    const message = stringifyError(value);
    return /(?:maximum number of turns|max(?:imum)? turns|turn limit)/i.test(message);
}
export async function runClaudeCodeAgent({ plan, model, logger, toolAdapter, signal, sdk: injectedSdk, verifier, }) {
    ...
    const allowedTools = toolAdapter?.allowedTools ??
        ...
        for await (const message of sdk.query({
            ...
            options: {
                ...
                systemPrompt: {
                    type: "preset",
                    preset: "claude_code",
                    append: "You are being evaluated. Do not edit repository files. Complete the browser task and emit the requested EVAL_RESULT line.",
                },
            },
        })) {
    ...
    return gradeExternalTrajectory({
        buildTrajectory: () => claudeCodeAdapter.fromHarnessResult({
            messages,
...
function buildClaudeCodeMetrics(resultMessage) {
    const tokenUsage = extractClaudeCodeTokenUsage(resultMessage);
    return {
...
function extractClaudeCodeTokenUsage(resultMessage) {
...
function sumModelUsage(resultMessage, key) {
    if (!isRecord(resultMessage?.modelUsage))
        return 0;
    let total = 0;
...
function resolveClaudeCodeStatus(resultMessage, iterationError) {
    if (isClaudeCodeMaxTurnsError(iterationError) ||
...
function buildClaudeCodeStopReason(resultMessage, iterationError) {
    if (iterationError)
        return stringifyError(iterationError);
...
function buildClaudeCodeTranscript(messages) {
    return messages
        .map((message) => summarizeClaudeCodeMessage(message).detail)
        .filter((detail) => Boolean(detail))
        .join("\n");
}
function logClaudeCodeMessage(logger, message) {
...
function summarizeClaudeCodeMessage(message) {
...
async function loadClaudeAgentSdk() {
    try {
        ...
        throw new EvalsError(`Claude Code harness requires ${CLAUDE_AGENT_SDK_PACKAGE}. Install it in packages/evals before running --harness claude_code. ${error instanceof Error ? error.message : String(error)}`);
    }
}
