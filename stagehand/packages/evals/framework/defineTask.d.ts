 ...
 */
import type { BenchTaskContext, BenchTaskMeta, CoreTaskContext, TaskDefinition, TaskMeta, TaskResult } from "./types.js";
/**
 * Define a core tier task (deterministic, no LLM).
 * Core tasks receive { page, assert, metrics, logger } and throw on failure.
 */
export declare function defineCoreTask(meta: TaskMeta, fn: (ctx: CoreTaskContext) => Promise<void | TaskResult>): TaskDefinition;
/**
 * Define a bench tier task (with LLM and evaluator).
 * Bench tasks receive { v3, agent, page, logger, input, ... } and return TaskResult.
 */
export declare function defineBenchTask(meta: BenchTaskMeta, fn: (ctx: BenchTaskContext) => Promise<void | TaskResult>): TaskDefinition;
/**
 ...
 */
export declare function defineTask(meta: TaskMeta | BenchTaskMeta, fn: (ctx: CoreTaskContext | BenchTaskContext) => Promise<void | TaskResult>): TaskDefinition;
