...
function getTaskBasename(taskName) {
    if (!taskName.includes("/"))
        return taskName;
    const parts = taskName.split("/");
    return parts[parts.length - 1] ?? taskName;
}
function getExtraCategories(taskName) {
    return (EXTRA_CATEGORIES[taskName] ??
...
function getTierRoots(tasksRoot, tier) {
    if (tier === "bench") {
 ...
 */
function parseTaskPath(filePath, tierRoot, tier) {
 ...
 */
async function loadTaskModule(filePath, expectedName) {
 ...
 */
export async function discoverTasks(tasksRoot, eager = false) {
    const tasks = [];
    ...
    for (const tier of TIERS) {
        ...
        for (const tierRoot of tierRoots) {
            ...
            for (const filePath of files) {
                    ...
                    try {
                        ...
                        if (result.definition) {
                            ...
                            if (meta.name)
                                taskName = meta.name;
                            if (meta.categories)
                                extraCategories = meta.categories;
                            if (meta.tags)
                                tags = meta.tags;
                        ...
                        console.warn(`[discovery] Failed to load task module ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
                        continue;
                    }
                }
                ...
                if (!byTier.has(parsed.tier))
                    byTier.set(parsed.tier, []);
 ...
 */
export function resolveTarget(registry, target) {
    if (!target) {
    ...
    if (TIERS.includes(target)) {
        return registry.byTier.get(target) ?? [];
    }
