...
export function createAssertHelpers() {
    return {
        equals(actual, expected, message) {
            try {
                ...
                throw new AssertionError(message ??
                    `Expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`, actual, expected);
            }
        },
        matches(actual, pattern, message) {
            if (!pattern.test(actual)) {
                throw new AssertionError(message ?? `Expected "${actual}" to match ${pattern}`, actual, pattern);
            }
        },
        includes(haystack, needle, message) {
            if (!haystack.includes(needle)) {
                throw new AssertionError(message ?? `Expected "${haystack}" to include "${needle}"`, haystack, needle);
            }
        },
        ...
        falsy(value, message) {
            if (value) {
                throw new AssertionError(message ?? `Expected falsy value, got ${JSON.stringify(value)}`, value, false);
            }
        },
