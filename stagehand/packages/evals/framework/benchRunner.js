...
function withBenchSessionUrls(result, ctx) {
    if (!ctx)
        return result;
    return {
        ...result,
...
export async function executeBenchTask(input, task, options) {
    ...
    const row = buildBenchMatrixRow(task, input.modelName, options, input.params, input.isCUA, input.agentMode);
    let cleanup = async () => { };
    ...
    try {
        ...
        if (taskModule.definition) {
            ...
            return withBenchSessionUrls((await taskModule.definition.fn(ctx)), harnessCtx);
        }
