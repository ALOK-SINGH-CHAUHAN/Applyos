...
export function resolveDefaultCoreStartupProfile(toolSurface, environment) {
    switch (toolSurface) {
        case "browse_cli":
    ...
    throw new Error(`No default startup profile for tool "${toolSurface}" in environment "${environment}"`);
}
/**
...
export async function buildCoreContext(options = {}) {
    ...
    const startupProfile = options.startupProfile ??
        resolveDefaultCoreStartupProfile(toolSurface, environment);
    if (!tool.supportedStartupProfiles.includes(startupProfile)) {
        throw new Error(`Tool surface "${toolSurface}" does not support startup profile "${startupProfile}".`);
    }
 ...
 */
export async function buildBenchContext(options) {
    const logger = options.logger ?? new EvalLogger();
