 ...
 */
export function generateRunToken(now = new Date(), entropy = randomBytes(RUN_TOKEN_ENTROPY_BYTES).toString("hex")) {
 ...
 */
export function resolveUnambiguousModel(models) {
...
export function buildTrajectoryGroupSlug(opts) {
    ...
    if (opts.model)
        parts.push(opts.model);
    if (opts.runToken)
        parts.push(opts.runToken);
 ...
 */
export function resolveTrajectoryDir(root, taskId, runId, group = resolveTrajectoryGroup()) {
 ...
 */
export async function reserveTrajectoryDir(root, taskId, runId, group) {
 ...
 */
export function resolveTrajectoryRoot() {
    return (process.env.EVAL_TRAJECTORY_ROOT ||
 ...
 */
export async function writeExperimentLink(root, group, link, opts) {
    if (!(opts?.persist ?? shouldPersistTrajectory(undefined)))
        return;
...
export function trajectoryRunMetadata() {
    const fields = {
 ...
 */
export async function writeTrajectoryMetadata(directory, extra = {}) {
