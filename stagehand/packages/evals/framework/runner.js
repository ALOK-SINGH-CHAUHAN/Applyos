...
import { buildTrajectoryGroupSlug, generateRunToken, resolveUnambiguousModel, resolveTrajectoryRoot, writeExperimentLink, } from "./trajectoryGroup.js";
...
async function executeCoreTask(_input, task, options) {
    ...
    try {
        ...
        const startupResult = await tracedSpan(async () => buildCtx({
            logger,
        ...
        result = await tracedSpan(async () => {
            ...
            if (taskModule.legacyFn) {
                throw new EvalsError(`Legacy core task exports are not supported in the adapter-backed core runner: ${task.filePath}`);
            }
...
export async function runEvals(options) {
    ...
    const effectiveCoreToolSurface = hasCoreOnly
        ...
        : undefined;
    const effectiveCoreStartupProfile = hasCoreOnly && effectiveCoreToolSurface
    ...
    if (options.provider)
        process.env.EVAL_PROVIDER = options.provider;
    ...
    const onAggressiveAbort = async () => {
        if (readAbortMode(options.signal) !== "aggressive")
            return;
    ...
    const evalResult = await Eval(braintrustProjectName, {
        ...
        task: async (input) => {
            ...
            const resolvedTask = options.registry.byName.get(input.name) ??
                (input.name.includes("/")
                    ? undefined
    ...
    const summaryResults = evalResult.results.map((result) => {
        ...
        const categories = Array.isArray(result.metadata?.categories)
            ? result.metadata.categories.filter((category) => typeof category === "string")
            : undefined;
        return {
            input: result.input,
            output,
    ...
    await generateSummary(summaryResults, resolvedExperimentName, resolvedExperimentUrl, evalResult.summary?.scores);
