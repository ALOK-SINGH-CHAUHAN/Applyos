...
export function buildExternalHarnessTaskPlan(input) {
    ...
    if (input.name === "agent/webvoyager") {
        ...
        if (!startUrl || !instruction) {
            throw new EvalsError(`Missing WebVoyager params for external harness: expected web and ques.`);
        }
        return {
            dataset: "webvoyager",
    ...
    if (input.name === "agent/onlineMind2Web") {
        ...
        if (!startUrl || !instruction) {
            throw new EvalsError(`Missing onlineMind2Web params for external harness: expected website and confirmed_task.`);
        }
        return {
    ...
    if (input.name === "agent/odysseysbench") {
        const instruction = readString(params, "confirmed_task");
        if (!instruction) {
            throw new EvalsError(`Missing OdysseysBench params for external harness: expected confirmed_task.`);
        }
        return {
    ...
    throw new EvalsError(`External harness "${input.name}" is not supported yet. Supported: agent/webvoyager, agent/onlineMind2Web, agent/webtailbench, agent/odysseysbench.`);
}
