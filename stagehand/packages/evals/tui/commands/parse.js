 ...
 */
import { DEFAULT_BENCH_HARNESS, parseBenchHarness, } from "../../framework/benchTypes.js";
const SUCCESS_MODES = new Set([
    "outcome",
    "process",
    "both",
]);
...
function normalizeEnvironment(raw, source) {
...
export function parseAgentModes(raw) {
    const modes = raw
        .split(",")
        .map((part) => part.trim())
        .filter(Boolean)
...
function parseFilter(raw) {
    ...
    if (!/^[A-Za-z][A-Za-z0-9_]*$/.test(key)) {
        throw new Error("--filter key must start with a letter and contain only letters, numbers, or underscores");
    }
    return [key, value];
}
 ...
 */
export function applyBenchmarkShorthand(target, flags) {
    const envOverrides = {};
    if (target === "all") {
    ...
    if (LEGACY_ONLY_BENCHMARKS.has(benchmarkName)) {
        if (!flags.legacy) {
            throw new Error(`Benchmark "${benchmarkName}" is legacy-only. Use --legacy or choose one of: ${[...SUPPORTED_BENCHMARKS].join(", ")}.`);
        }
    }
    if (!SUPPORTED_BENCHMARKS.has(benchmarkName) &&
        !LEGACY_ONLY_BENCHMARKS.has(benchmarkName)) {
        throw new Error(`Unknown benchmark "${benchmarkName}". Supported: ${[...SUPPORTED_BENCHMARKS].join(", ")}.`);
    }
...
export function resolveRunOptions(flags, defaults, env, core = {}) {
    const parseIntEnv = (value) => {
        if (!value)
            return undefined;
        return parsePositiveInteger(value, "environment value");
    };
    const rawEnv = flags.env ??
    ...
    const { target, datasetFilter: shorthandDatasetFilter, envOverrides, } = applyBenchmarkShorthand(flags.target, flags);
    ...
    const successMode = flags.success ??
        (SUCCESS_MODES.has(envSuccess)
            ? envSuccess
            : "outcome");
 ...
 */
export async function withEnvOverrides(overrides, fn) {
