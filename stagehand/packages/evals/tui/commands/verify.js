...
import { V3, V3Evaluator, loadTrajectoryFromDisk, nextResultFilename, } from "@browserbasehq/stagehand";
...
export async function handleVerify(args) {
    ...
    if (!parsed.json) {
        console.log(`  ${green("✓")} ${trajectory.steps.length} steps · status=${trajectory.status} · task=${trajectory.task.id}`);
    }
    ...
    if (!parsed.json) {
        console.log(`${cyan("▸")} running V3Evaluator.verify()${parsed.model ? ` with model=${parsed.model}` : ""}`);
    }
    ...
    console.log(`${bold("Result")}  outcomeSuccess=${result.outcomeSuccess}  processScore=${processScore}`);
    const perCriterion = result.perCriterion ?? [];
    const evidenceInsufficient = result.evidenceInsufficient ?? [];
    console.log(`${dim("       ")} criteria=${perCriterion.length}  evidenceInsufficient=${evidenceInsufficient.length}`);
...
async function assertTrajectoryDir(dir) {
    try {
