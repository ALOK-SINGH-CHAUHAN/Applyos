...
export function resolveConfigPath(entryDir) {
    return path.join(entryDir, "evals.config.json");
}
export function readConfig(entryDir) {
...
export function writeConfig(entryDir, config) {
    const configPath = resolveConfigPath(entryDir);
...
export function printConfig(entryDir) {
    ...
    console.log(`    ${cyan("agentModes")}   ${defaults.agentModes?.length
    ...
    const overrides = [];
    if (env.EVAL_ENV)
        overrides.push(`EVAL_ENV=${env.EVAL_ENV}`);
    if (env.EVAL_MODELS)
        overrides.push(`EVAL_MODELS=${env.EVAL_MODELS}`);
    if (env.EVAL_PROVIDER)
        overrides.push(`EVAL_PROVIDER=${env.EVAL_PROVIDER}`);
    if (env.USE_API)
        overrides.push(`USE_API=${env.USE_API}`);
...
export async function handleConfig(args, entryDir) {
    if (args.length === 0) {
