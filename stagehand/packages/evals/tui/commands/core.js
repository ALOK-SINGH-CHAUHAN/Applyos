...
import { readConfig, writeConfig, resolveConfigPath, } from "./config.js";
const VALID_KEYS = ["tool", "startup"];
export async function handleCore(args, entryDir) {
    const sub = args[0];
    if (!sub) {
    ...
    if (sub === "setup") {
        console.log(dim("  evals config core setup — interactive wizard coming soon. For now use " +
...
function printCoreUsage() {
    console.log(dim("  Usage: config core [set <k> <v>|reset [key]|path|setup]"));
}
export function printCoreConfig(entryDir) {
...
async function setCoreKey(entryDir, key, value) {
    ...
    if (key === "tool") {
        ...
        if (core.startup) {
            const newTool = getCoreTool(value);
            if (!newTool.supportedStartupProfiles.includes(core.startup)) {
...
function resetCoreKey(entryDir, key) {
    const config = readConfig(entryDir);
    if (!key) {
...
function pruneCore(core) {
    const pruned = {};
    if (core.tool)
        pruned.tool = core.tool;
    if (core.startup)
        pruned.startup = core.startup;
