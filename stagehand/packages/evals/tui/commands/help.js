...
function row(left, right, width = HELP_COL_WIDTH) {
...
export function printRunHelp() {
    print([
        ...
        "",
        row(`${cyan("--harness")} ${dim("<name>")}`, `Bench harness ${gray("(stagehand | claude_code | codex)")}`),
        ...
        row(`${cyan("--success")} ${dim("<mode>")}`, `Rubric success mode ${gray("(outcome | process | both)")}`),
        ...
        `  ${bold("Inspect:")}`,
        "",
        row(cyan("--preview"), `Print a human-readable plan ${gray("(combinations + tasks)")} and exit`),
        "",
        `  ${bold("Escape hatch:")}`,
        "",
...
export function printConfigCoreHelp() {
    print([
        "",
...
export function printExperimentsHelp(subcommand) {
    if (subcommand === "list") {
        print([
            "",
