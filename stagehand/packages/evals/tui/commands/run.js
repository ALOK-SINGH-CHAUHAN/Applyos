...
function uniqueStringValues(rows, key, options = {}) {
...
function buildRunTargetLabel(options) {
    return options.target ?? options.normalizedTarget ?? "bench";
}
function buildPlanLine(options, matrix) {
    ...
    const hasDatasetCases = uniqueStringValues(matrix, "dataset", {
        requireTruthy: true,
    }).length > 0;
    const baseLabel = hasDatasetCases || !canFactorCleanly || baseCount !== taskCount
        ? "case"
        : "task";
    ...
    if (canFactorCleanly) {
        ...
        if (harnessCount > 1)
            factors.push(formatCount(harnessCount, "harness"));
...
function buildRunContextLine(options, tasks, matrix) {
...
function isExplicitLegacyOnlyTarget(target) {
    return Boolean(target && LEGACY_ONLY_BENCHMARK_TARGETS.has(target));
}
function splitLegacyOnlyTasks(tasks) {
    const runnableTasks = [];
    const skippedTasks = [];
...
export async function runCommand(options, registry, signal) {
    ...
    try {
        ...
        if (planMode) {
            await emitDryRun(options, [], registry, err.message);
            process.exitCode = 1;
            return;
        }
        throw err;
    ...
    if (options.useApi &&
        ...
        throw new Error(`Harness "${options.harness}" does not support --api. Use --harness stagehand for API-backed bench runs.`);
    }
    if (planMode) {
        await emitDryRun(options, tasks, registry, undefined, skippedTasks);
        return;
    }
    if (!canExecuteBenchHarness(options.harness) &&
        tasks.some((t) => t.tier === "bench")) {
        throw new Error(`Harness "${options.harness}" is dry-run only for now. Use --harness stagehand, --harness claude_code, or --harness codex for executable bench runs.`);
    }
    ...
    if (skippedTasks.length > 0) {
        console.log(`  ${bold("Skipped:")} ${skippedTasks.length} legacy-only task(s) ${dim(skippedTasks.map((task) => task.name).join(", "))}`);
    }
...
export function deriveCategoryFilter(registry, normalizedTarget) {
    if (!normalizedTarget)
        return undefined;
...
export function canExecuteBenchHarness(harness) {
 ...
 */
async function emitDryRun(options, tasks, registry, error, skippedTasks = []) {
...
async function buildDryRunMatrix(options, tasks, registry) {
    return withEnvOverrides(options.envOverrides, async () => {
        ...
        if (benchTasks.length > 0) {
            ...
            for (const testcase of testcases) {
                const task = registry.byName.get(testcase.input.name) ??
                    (testcase.input.name.includes("/")
                        ? undefined
                        : registry.byName.get(`agent/${testcase.input.name}`));
                const row = task
                    ? buildBenchMatrixRow(task, testcase.input.modelName, {
                        ...options,
                        coreToolSurface: options.coreToolSurface,
                        coreStartupProfile: options.coreStartupProfile,
                    }, testcase.input.params, testcase.input.isCUA, testcase.input.agentMode)
                    : undefined;
                rows.push(sortKeys({
                    tier: testcase.metadata.tier ?? "bench",
                    task: testcase.metadata.task ?? testcase.input.name,
                    category: testcase.metadata.task_category ??
                        testcase.metadata.category ??
                        null,
                    dataset: testcase.metadata.dataset ?? null,
                    model: testcase.input.modelName,
