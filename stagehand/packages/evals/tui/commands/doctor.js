...
import { resolveKey, snapshotEnv } from "../welcomeStatus.js";
...
export function printDoctorHelp() {
    const HELP_COL = 28;
    const row = (left, right) => `    ${padRight(left, HELP_COL)} ${right}`;
    console.log([
        "",
...
function summarizeConfig(entryDir) {
    let env = null;
    let trials = null;
    let concurrency = null;
    let coreTool = null;
    let coreStartup = null;
    try {
        const c = readConfig(entryDir);
        env = c.defaults.env ?? null;
        trials = c.defaults.trials ?? null;
        concurrency = c.defaults.concurrency ?? null;
...
async function summarizeDiscovery() {
    const root = getRuntimeTasksRoot();
    try {
        const registry = await discoverTasks(root, false);
 ...
 *   ok    — otherwise.
 */
function computeVerdict(keys, config, discovery) {
    const reasons = [];
    if (!discovery.ok) {
        reasons.push(`Discovery failed: ${discovery.error ?? "unknown error"}`);
    }
    const zeroProviders = keys.openai.state === "missing" &&
    ...
    const bothBBMissing = keys.browserbase.apiKey === "missing" &&
        keys.browserbase.projectId === "missing";
    if (envIsBrowserbase && bothBBMissing) {
        reasons.push("env=browserbase but both BROWSERBASE_API_KEY and BROWSERBASE_PROJECT_ID are missing.");
    }
    if (!discovery.ok || zeroProviders || (envIsBrowserbase && bothBBMissing)) {
        return { verdict: "fail", reasons };
    }
    const partialBB = (keys.browserbase.apiKey === "set" &&
        keys.browserbase.projectId === "missing") ||
...
async function buildReport(entryDir) {
    const runtime = {
...
// Renderers
// ---------------------------------------------------------------------------
function keyRow(label, entry, note) {
    const value = entry.state === "set"
...
function renderHuman(report) {
    ...
    console.log(`    ${padRight("Stagehand", 15)} ${r.runtime.stagehand ?? gray("(unknown)")}     ${dim("(packages/core/package.json)")}`);
    ...
    console.log(`    ${padRight("core.tool", 22)} ${r.config.core.tool
    ...
    if (r.discovery.ok) {
        console.log(`    ${padRight("Tasks", 22)} ${cyan(String(r.discovery.total))}  ${dim(`(core: ${r.discovery.core} · bench: ${r.discovery.bench})`)}`);
    ...
    console.log(keyRow(googleLabel, {
        state: r.keys.google.state,
        source: r.keys.google.source,
    }));
    console.log(`    ${padRight("BROWSERBASE_API_KEY", 30)} ${r.keys.browserbase.apiKey === "set" ? green("✓ set") : red("✗ missing")}${r.keys.browserbase.viaAlias ? `        ${dim("(via BB_API_KEY)")}` : ""}`);
    console.log(`    ${padRight("BROWSERBASE_PROJECT_ID", 30)} ${r.keys.browserbase.projectId === "set" ? green("✓ set") : red("✗ missing")}`);
    ...
    console.log("");
    if (r.verdict !== "ok") {
        console.log(`  ${dim("To set keys: edit")} ${cyan(path.join(getPackageRootDir(), ".env"))} ${dim("or export them in your shell.")}`);
        console.log("");
    }
}
...
// Probe (hidden)
// ---------------------------------------------------------------------------
async function runOpenAIProbe(keys) {
...
// Entry point
// ---------------------------------------------------------------------------
export async function handleDoctor(args, entryDir) {
