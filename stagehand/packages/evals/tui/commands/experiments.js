...
import { benchCaseDiffs, collectExperimentMetrics, detectCompareMode, listRecentExperiments, resolveExperimentAcrossProjects, resolveExperimentProjectsAcrossProjects, findLeaderIndex, sharedMetricKeys, sharedBenchCaseKeys, sharedTaskNames, summarizeBenchAgentConfigs, } from "../../lib/braintrust-report.js";
...
export async function handleExperiments(args) {
...
async function handleList(args) {
    ...
    for (const section of rows) {
        ...
        for (const experiment of section.experiments) {
            ...
            const passRate = experiment.passScore !== undefined
                ? formatRecentPassRate(experiment)
                : dim(padRight("—", 7));
            const duration = experiment.durationSeconds !== undefined
...
async function handleShow(args) {
    ...
    console.log(`  ${bold(experiment.mode === "bench" ? "Cases:" : "Tasks:")} ${experiment.passedTasks}/${experiment.totalTasks}`);
...
async function handleCompare(args) {
    ...
    const projectArgs = projects.length === 1
        ? ["--project", projects[0]]
        : [
            "--project-map",
    ...
    await new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            reject(new Error(`Compare report exited with code ${code ?? 1}${details ? `\n${details}` : ""}`));
        });
    });
}
...
function looksLikeExperimentId(value) {
    return (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(value) || /^[a-z][a-z0-9_-]*-[a-f0-9]{4,}$/i.test(value));
}
async function resolveCompareInputs(options) {
    const inputs = options.project
        ...
        : options.experiments;
    const resolved = await resolveExperimentProjectsAcrossProjects(options.project ? [options.project] : DEFAULT_LIST_PROJECTS, inputs);
...
function formatExperimentArg(input) {
    if (input.label === input.experiment)
        return input.experiment;
    return `${input.label}=${input.experiment}`;
}
function formatPassRate(experiment, color = true) {
    const pct = `${(experiment.passScore * 100).toFixed(1)}%`;
    if (!color)
        return pct;
    if (experiment.passScore >= 0.8)
        return green(pct);
    if (experiment.passScore >= 0.5)
        return pct;
    return red(pct);
}
function formatRecentPassRate(experiment) {
...
function renderHeadlessCompareSummary(project, reportPath, dataPath) {
