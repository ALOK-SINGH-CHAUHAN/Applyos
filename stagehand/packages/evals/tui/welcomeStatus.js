...
function loadPackageEnv() {
    ...
    if (process.env.EVALS_DISABLE_PACKAGE_ENV === "1")
        return {};
    if (packageEnvLoaded)
...
// ---------------------------------------------------------------------------
export function hasZeroProviderKeys(s) {
    return (s.openai.state === "missing" &&
        s.anthropic.state === "missing" &&
        s.google.state === "missing");
}
export function renderInlineWarning(s) {
    if (!hasZeroProviderKeys(s))
        return null;
 ...
 */
export function __resetPackageEnvCacheForTests() {
    cachedPackageEnv = null;
