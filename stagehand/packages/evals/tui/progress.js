 ...
 */
import { green, red, blue, gray, dim, bold, formatMs, padRight, separator, getTerminalWidth, truncateText, visibleLength, writeLine, writeRaw, } from "./format.js";
...
export class ProgressRenderer {
    ...
    onPass(taskName, model, durationMs) {
        const key = model ? `${taskName}:${model}` : taskName;
        this.tasks.set(key, {
            name: taskName,
            model,
        ...
        this.printRow(green("✓"), taskName, model, green("passed"), durationMs !== undefined ? dim(formatMs(durationMs)) : undefined);
    }
    onFail(taskName, model, error) {
        const key = model ? `${taskName}:${model}` : taskName;
    ...
    printSummary() {
        ...
        writeLine(`  ${bold("Results:")} ${green(`${this.passed} passed`)}, ${red(`${this.failed} failed`)} ${dim(`(${total} total)`)}`);
        if (total > 0) {
    ...
    printRow(icon, taskName, model, status, duration) {
    ...
    formatAnimatedRow(task) {
        switch (task.status) {
            case "running":
                return this.buildRow(blue(DOTS2_FRAMES[this.frameIndex % DOTS2_FRAMES.length]), task.name, task.model, gray("running"));
            case "passed":
                return this.buildRow(green("✓"), task.name, task.model, green("passed"), task.durationMs !== undefined
                    ? dim(formatMs(task.durationMs))
                    : undefined);
            case "failed":
            case "error":
                return this.buildRow(red("✗"), task.name, task.model, red("failed"), task.error ? gray(truncateText(task.error, 18)) : undefined);
            case "pending":
            default:
                return this.buildRow(gray("⠁"), task.name, task.model, gray("pending"));
        }
    }
    buildRow(icon, taskName, model, status, duration) {
    ...
    buildOverflowRow(hiddenCount, total) {
    ...
    getRowLayout(hasModel, status, duration) {
        const width = getTerminalWidth();
        const contentWidth = Math.max(32, width - 6);
        const statusWidth = Math.max(7, visibleLength(status));
        const durationWidth = duration ? visibleLength(duration) + 1 : 0;
        const longestModel = this.getLongestModelLength();
        const preferredModelWidth = hasModel && longestModel > 0
    ...
    getLongestModelLength() {
        return Math.max(0, ...[...this.tasks.values()]
            .map((task) => task.model)
