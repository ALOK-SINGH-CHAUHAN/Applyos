...
export function readWelcomeMeta(entryDir) {
    try {
...
export function isFirstRun(entryDir) {
 ...
 */
export function markFirstRunComplete(entryDir) {
    try {
        const config = readConfig(entryDir);
        if (config._meta?.firstRunCompletedAt)
            return;
        config._meta = {
            ...(config._meta ?? {}),
