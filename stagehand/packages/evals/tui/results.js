 ...
 */
import { bold, green, red, dim, cyan, gray, separator, padRight, getTerminalWidth, } from "./format.js";
export function printResultsTable(results) {
    ...
    console.log(`  ${bold(padRight("Task", taskWidth))} ${bold(padRight("Model", modelWidth))} ${bold(padRight("Result", resultWidth))}`);
    console.log(separator());
    for (const [name, taskResults] of byTask) {
        for (const r of taskResults) {
            ...
            console.log(`  ${padRight(name, taskWidth)} ${dim(padRight(r.input.modelName, modelWidth))} ${result}`);
        }
    }
    console.log(separator());
    printModelSummary(results, true);
}
export function printModelSummary(results, leadingBlankLine = false) {
    ...
    for (const [model, stats] of modelStats) {
        ...
        console.log(`    ${padRight(model, summaryWidth)} ${color(`${pct}%`)} ${gray(`(${stats.passed}/${stats.total})`)}`);
    }
    console.log("");
}
