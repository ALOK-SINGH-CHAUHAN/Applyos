...
function browserbaseLabel(s) {
    if (s.browserbase.apiKey === "set" && s.browserbase.projectId === "set") {
        return green("✓");
    }
    if (s.browserbase.apiKey === "missing" &&
 ...
 */
export function printExtendedWelcome(ctx) {
    const lines = [];
    lines.push("");
    lines.push(`  ${bold("Welcome to Stagehand evals.")}`);
    lines.push("");
    lines.push(`  ${dim("·")} Type a command (e.g. ${cyan("list")}) or a run target (e.g. ${cyan("act")}).`);
    lines.push(`  ${dim("·")} ${cyan("help")} for commands · ${cyan("evals doctor")} for env health · ${cyan("exit")} to quit.`);
    lines.push("");
    lines.push(`  ${bold("Health")}`);
    lines.push(`    AI:  ${providerLabel(ctx.snapshot)}     BB: ${browserbaseLabel(ctx.snapshot)}     Braintrust: ${tagIcon(ctx.snapshot.braintrust.state)}`);
    ...
    lines.push(`    ${cyan("run agent")}                  ${dim("# run the agent category once (env=local)")}`);
    lines.push(`    ${cyan("experiments list")}           ${dim("# recent Braintrust runs (needs BRAINTRUST_API_KEY)")}`);
