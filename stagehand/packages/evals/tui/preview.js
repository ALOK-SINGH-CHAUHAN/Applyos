 ...
 * the run.
 */
import { bold, cyan, dim, gray, padRight, separator, getTerminalWidth, red, yellow, } from "./format.js";
/**
...
function renderHeader(p) {
    ...
    console.log(`  ${bold("Target:")} ${cyan(String(target))}  ${dim("→")}  ${tierLabel} (${p.tasks.length} task${p.tasks.length === 1 ? "" : "s"})`);
    ...
    if (env)
        headerBits.push(`${bold("Env:")} ${cyan(env)}`);
    ...
    if (opts.useApi === true)
        headerBits.push(`${bold("API:")} ${yellow("on")}`);
    if (opts.model)
        headerBits.push(`${bold("Model override:")} ${opts.model}`);
    if (opts.provider)
        headerBits.push(`${bold("Provider:")} ${opts.provider}`);
...
function uniqueTiers(matrix) {
    const seen = new Set();
    for (const row of matrix)
        seen.add(String(row.tier ?? "?"));
    return [...seen];
}
// ---------------------------------------------------------------------------
// Skipped tasks
// ---------------------------------------------------------------------------
function renderSkipped(p) {
    if (p.skippedTasks.length === 0)
        return;
    console.log(`  ${bold("Skipped:")} ${p.skippedTasks.length} legacy-only task(s) ${dim(p.skippedTasks.join(", "))}`);
    console.log("");
}
export function buildCombinations(matrix) {
    if (matrix.length === 0)
        return { columns: [], rows: [] };
...
function renderCombinations(p) {
    ...
    const colWidths = columns.map((col, i) => Math.max(headerCells[i].length, ...rows.map((r) => formatCell(r.values[col]).length)));
    ...
    const sortedRows = [...rows].sort((a, b) => columns
...
function renderFooter(p) {
    ...
    const combos = new Set(p.matrix.map((row) => {
        const shape = {};
        for (const key of Object.keys(row)) {
            if (HIDDEN_COLUMNS.has(key))
                continue;
