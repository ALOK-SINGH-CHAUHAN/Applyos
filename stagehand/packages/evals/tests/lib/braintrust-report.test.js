...
vi.mock("braintrust", () => ({
    loginToState: async () => {
        ...
        return {
            ...
            apiConn: () => ({
                get_json: async (endpoint, params) => {
...
import { clearBraintrustReportCache, collectExperimentMetrics, detectCompareMode, extractBenchCases, fetchManyExperimentData, inferExperimentMode, resolveExperimentProjectsAcrossProjects, sharedBenchCaseKeys, summarizeBenchAgentConfigs, } from "../../lib/braintrust-report.js";
describe("braintrust-report", () => {
    ...
    it("resolves compare inputs across bench and core projects independently", async () => {
        ...
        const resolved = await resolveExperimentProjectsAcrossProjects(["stagehand-dev", "stagehand-core-dev"], [
    ...
    it("uses per-input projects and reuses cached payloads across fetches", async () => {
        ...
        expect(braintrustMock.apiCalls.filter((call) => call.endpoint === "/experiment-comparison2")).toHaveLength(2);
    ...
    it("scopes cached Braintrust payloads by API key", async () => {
        ...
        await fetchManyExperimentData("stagehand-dev", [{ label: "A", experiment: "exp" }], { apiKey: "key-a" });
        await fetchManyExperimentData("stagehand-dev", [{ label: "A", experiment: "exp" }], { apiKey: "key-b" });
        expect(braintrustMock.loginCalls).toBe(2);
        expect(braintrustMock.apiCalls.filter((call) => call.endpoint === "/v1/experiment")).toHaveLength(2);
        expect(braintrustMock.apiCalls.filter((call) => call.endpoint === "/experiment-comparison2")).toHaveLength(2);
        expect(braintrustMock.fetchedCalls).toHaveLength(2);
    });
    it("honors explicit core tier before modelName heuristics", () => {
        expect(inferExperimentMode("stagehand-dev", [
            {
                is_root: true,
                input: {
                    name: "act/login",
    ...
    it("detects bench/core modes and rejects mixed comparisons", () => {
        ...
        expect(() => detectCompareMode([core, bench])).toThrow(/Cannot compare core and bench experiments together yet/);
    });
...
function makeExperimentData(overrides) {
    return {
        label: "Experiment",
        experimentName: "exp",
        experimentId: "id",
