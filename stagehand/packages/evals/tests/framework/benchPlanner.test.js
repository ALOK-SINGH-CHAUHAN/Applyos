...
describe("benchPlanner", () => {
    it("builds stagehand matrix rows by default", () => {
        const task = makeTask();
        const row = buildBenchMatrixRow(task, "openai/gpt-4.1-mini", {
    ...
    it("marks explicit CUA-only model overrides as CUA", () => {
        const cuaModel = "openai/computer-use-preview";
    ...
    it("runs configured non-CUA agent models in dom and hybrid modes", async () => {
        ...
        expect(testcases).toHaveLength(2);
        expect(testcases.map((testcase) => testcase.input.agentMode).sort()).toEqual(["dom", "hybrid"]);
        expect(testcases.every((testcase) => testcase.input.modelName === "openai/gpt-4.1-mini")).toBe(true);
    });
    it("can run CUA-capable models in requested dom and hybrid modes", async () => {
        ...
        expect(testcases.map((testcase) => testcase.input.agentMode).sort()).toEqual(["dom", "dom", "hybrid", "hybrid"]);
        expect(testcases
            .filter((testcase) => testcase.input.modelName === "google/gemini-3-flash-preview")
            .map((testcase) => testcase.input.agentMode)
            .sort()).toEqual(["dom", "hybrid"]);
    });
    ...
    it("rejects unsupported Claude Code tasks from broad targets", async () => {
        const generate = () => withEnvOverrides({
            EVAL_MAX_K: "1",
            EVAL_WEBVOYAGER_LIMIT: "1",
        }, async () => generateBenchTestcases([
            makeTask(),
        ...
        await expect(generate()).rejects.toThrow('Harness "claude_code" only supports agent benchmark suites');
    ...
    it("generates direct WebTailBench suite testcases from source datasets", async () => {
        ...
        const rowCategory = testcases[0].input.params?.category;
