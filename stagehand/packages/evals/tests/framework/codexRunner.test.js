...
import { buildCodexPrompt, normalizeCodexModel, parseCodexResult, runCodexAgent, } from "../../framework/codexRunner.js";
...
describe("codex runner helpers", () => {
    ...
    it("parses legacy EVAL_RESULT marker JSON", () => {
        expect(parseCodexResult('assistant text\nEVAL_RESULT: {"success":true,"summary":"done"}')).toMatchObject({
            success: true,
            summary: "done",
        });
    });
    it("streams events into a task result and reports token metrics", async () => {
        let capturedThreadOptions;
