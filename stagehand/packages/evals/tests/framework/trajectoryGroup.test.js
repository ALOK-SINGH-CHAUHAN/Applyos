...
import { buildTrajectoryGroupSlug, generateRunToken, resolveTrajectoryGroup, resolveUnambiguousModel, sanitizeSlug, writeExperimentLink, } from "../../framework/trajectoryGroup.js";
const tempDirs = [];
...
afterEach(async () => {
    ...
    if (persistEnv === undefined)
        ...
        process.env.VERIFIER_PERSIST_TRAJECTORIES = persistEnv;
...
describe("buildTrajectoryGroupSlug", () => {
    it("appends the run token as the last part", () => {
        expect(buildTrajectoryGroupSlug({
...
describe("resolveUnambiguousModel", () => {
    ...
    it("omits two distinct models", () => {
        expect(resolveUnambiguousModel([
            "openai/gpt-4.1-mini",
    ...
    it("returns the only distinct model when duplicated", () => {
        expect(resolveUnambiguousModel(["openai/gpt-4.1-mini", "openai/gpt-4.1-mini"])).toBe("openai/gpt-4.1-mini");
    });
    it("ignores core sentinels beside one real model", () => {
        expect(resolveUnambiguousModel(["none", "openai/gpt-4.1-mini", " NONE "])).toBe("openai/gpt-4.1-mini");
    });
});
describe("writeExperimentLink", () => {
     ...
     */
    async function recordTrajectory(root, group) {
    ...
    it("writes when persistence is explicitly enabled", async () => {
        ...
        await expect(fs.readdir(path.join(root, "group-1")).then((d) => d.sort())).resolves.toEqual(["experiment.json", "task-1"]);
    });
    it("keeps a separate experiment.json per run token (no clobber)", async () => {
        process.env.VERIFIER_PERSIST_TRAJECTORIES = "1";
        const root = await makeTempDir();
        const groupFor = (runToken) => buildTrajectoryGroupSlug({
            experimentName: "agent",
            model: "openai/gpt-4.1-mini",
            runToken,
        });
        ...
        await expect(fs.readdir(root).then((d) => d.sort())).resolves.toEqual([firstGroup, secondGroup].sort());
        await expect(readJson(path.join(root, firstGroup, "experiment.json"))).resolves.toMatchObject({ braintrustExperimentId: "first" });
        await expect(readJson(path.join(root, secondGroup, "experiment.json"))).resolves.toMatchObject({ braintrustExperimentId: "second" });
    });
    it("does not treat a requested model as resolved run metadata", async () => {
        ...
        await recordTrajectory(root, "group-1");
        await writeExperimentLink(root, "group-1", { braintrustExperimentId: "x" });
        await expect(readJson(path.join(root, "group-1", "experiment.json"))).resolves.not.toHaveProperty("model");
    });
});
