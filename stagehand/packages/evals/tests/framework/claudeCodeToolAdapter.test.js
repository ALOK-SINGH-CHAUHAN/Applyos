...
import { getBrowseCliAllowedTools, getBrowseCliToolMetadata, insertAfterFrontmatter, isAllowedBrowseCommand, installBrowseSkill, resolveClaudeCodeStartupProfile, resolveClaudeCodeToolSurface, waitForCdpEvent, } from "../../framework/claudeCodeToolAdapter.js";
import { resolveCodexStartupProfile, resolveCodexToolSurface, } from "../../framework/codexToolAdapter.js";
...
describe("claude code tool adapter resolution", () => {
    ...
    it("defaults browse_cli startup by environment", () => {
        ...
        expect(resolveClaudeCodeStartupProfile("browse_cli", "BROWSERBASE")).toBe("tool_create_browserbase");
    });
    it("supports code tool surfaces as Claude Code run tools", () => {
        ...
        expect(resolveClaudeCodeStartupProfile("playwright_code", "LOCAL")).toBe("runner_provided_local_cdp");
        expect(resolveClaudeCodeStartupProfile("playwright_code", "BROWSERBASE")).toBe("runner_provided_browserbase_cdp");
        ...
        expect(resolveClaudeCodeStartupProfile("cdp_code", "BROWSERBASE")).toBe("runner_provided_browserbase_cdp");
    });
    it("rejects unsupported Claude Code tool surfaces for now", () => {
        expect(() => resolveClaudeCodeToolSurface("understudy_code")).toThrow(/supports --tool browse_cli, playwright_code, or cdp_code/);
    });
    it("supports browse_cli as the first Codex tool surface", () => {
        ...
        expect(() => resolveCodexToolSurface("playwright_code")).toThrow(/Codex harness supports --tool browse_cli/);
    });
    ...
    it("installs the browse skill as a project skill", async () => {
        ...
        try {
            ...
            const skill = await fsp.readFile(path.join(cwd, ".claude", "skills", "browse", "SKILL.md"), "utf8");
    ...
    it("keeps the installed skill's frontmatter byte-identical to the source SKILL.md", async () => {
        const sourcePath = path.join(getRepoRootDir(), "packages", "cli", "skills", "browse", "SKILL.md");
        ...
        try {
            await installBrowseSkill(cwd);
            const installed = await fsp.readFile(path.join(cwd, ".claude", "skills", "browse", "SKILL.md"), "utf8");
    ...
    it("keeps abandoned CDP event waits from becoming unhandled rejections", async () => {
        ...
        try {
            const wait = waitForCdpEvent(connection, "session-1", "Page.frameNavigated", 1);
