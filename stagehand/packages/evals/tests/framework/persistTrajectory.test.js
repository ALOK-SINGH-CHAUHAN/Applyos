...
describe("persistAdapterTrajectory", () => {
    it("round-trips probe and agent image evidence through loadTrajectoryFromDisk", async () => {
        ...
        try {
            ...
            await expect(fs.readFile(path.join(directory, "screenshots", "probe", "1.png"))).resolves.toEqual(PROBE_PNG);
            await expect(fs.readFile(path.join(directory, "screenshots", "agent", "1.png"))).resolves.toEqual(AGENT_PNG);
            await expect(fs.readFile(path.join(directory, "scores", "result.json"), "utf8")).resolves.toContain('"outcomeSuccess": true');
            await expect(fs.readFile(path.join(directory, "task_data.json"), "utf8")).resolves.toContain('"result"');
            const loaded = await loadTrajectoryFromDisk(directory);
            const step = loaded.steps[0];
            const imageModality = step.agentEvidence.modalities.find((modality) => modality.type === "image");
            const textModality = step.agentEvidence.modalities.find((modality) => modality.type === "text");
