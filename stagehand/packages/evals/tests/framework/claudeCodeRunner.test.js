...
import { buildClaudeCodePrompt, isClaudeCodeMaxTurnsError, normalizeClaudeCodeModel, parseClaudeCodeResult, runClaudeCodeAgent, } from "../../framework/claudeCodeRunner.js";
...
describe("claude code runner helpers", () => {
    it("normalizes provider-prefixed models for Claude Code", () => {
        expect(normalizeClaudeCodeModel("anthropic/claude-sonnet-4-20250514")).toBe("claude-sonnet-4-20250514");
        expect(normalizeClaudeCodeModel("claude-opus-4-1")).toBe("claude-opus-4-1");
    });
    ...
    it("parses the final EVAL_RESULT JSON line", () => {
        expect(parseClaudeCodeResult('intermediate text\nEVAL_RESULT: {"success":true,"summary":"done","finalAnswer":"clicked"}')).toEqual({
            success: true,
            summary: "done",
            finalAnswer: "clicked",
    ...
    it("parses marked result JSON from the first line after the marker", () => {
        expect(parseClaudeCodeResult('assistant text\nEVAL_RESULT: {"success":true,"summary":"done"}\ntrailing sdk text')).toMatchObject({
            success: true,
            summary: "done",
        });
    });
