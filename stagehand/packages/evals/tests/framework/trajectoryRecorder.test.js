...
function recordSimpleStep(recorder, screenshot) {
    recorder.record({
        type: "screenshot",
        screenshot,
...
function recordFinalAnswer(recorder, opts) {
    recorder.record({
        type: "final_answer",
...
describe("TrajectoryRecorder", () => {
    it("assembles ordered callback events into trajectory steps", async () => {
        ...
        expect(trajectory.steps[0].agentEvidence.modalities).toEqual(expect.arrayContaining([
        ...
        expect(trajectory.steps[1].agentEvidence.modalities).toEqual(expect.arrayContaining([
            { type: "image", bytes: screenshot, mediaType: "image/png" },
        ]));
    ...
    it("persists trajectory files and evaluator results", async () => {
        ...
        await expect(fs.readdir(taskDir)).resolves.toEqual(expect.arrayContaining([
            "core.log",
            "metadata.json",
            "scores",
            "screenshots",
        ...
        await expect(fs.readFile(path.join(taskDir, "screenshots", "probe", "1.png"))).resolves.toEqual(screenshot);
        await expect(fs.readFile(path.join(taskDir, "screenshots", "probe", "final.png"))).resolves.toEqual(Buffer.from("final-screen"));
        await expect(fs.readFile(path.join(taskDir, "screenshots", "agent", "1.png"))).resolves.toEqual(screenshot);
        await expect(fs.readFile(path.join(taskDir, "scores", "result.json"), "utf8")).resolves.toContain('"outcomeSuccess": true');
    ...
    it("does not overwrite a previous run that reused the same runId", async () => {
        ...
        await expect(fs.readFile(path.join(firstDir, "screenshots", "agent", "1.png"))).resolves.toEqual(Buffer.from("first"));
        await expect(fs.readFile(path.join(secondDir, "screenshots", "agent", "1.png"))).resolves.toEqual(Buffer.from("second"));
    ...
    it("overlapping finish() and persistResult() share ONE reservation (single-flight)", async () => {
        ...
        await expect(fs.readdir(runDir)).resolves.toEqual(expect.arrayContaining(["trajectory.json", "scores"]));
    ...
    it("persistResult() before finish() keeps the score (result persistence is order-free)", async () => {
        ...
        await expect(fs.readFile(path.join(runDir, "scores", "result.json"), "utf8")).resolves.toContain('"outcomeSuccess": true');
        // ...and the trajectory files must still all be there.
        await expect(fs.readdir(runDir)).resolves.toEqual(expect.arrayContaining([
            "metadata.json",
            "scores",
            "task_data.json",
            "trajectory.json",
        ...
        await expect(fs.readdir(path.join(outputRoot, "test-group", "recorder-task"))).resolves.toEqual(["run-1"]);
    });
    ...
    it("writes under the group captured at construction, not the current env", async () => {
        ...
        await expect(fs.readdir(path.join(outputRoot, "experiment-a", "recorder-task"))).resolves.toEqual(["run-1"]);
    });
});
