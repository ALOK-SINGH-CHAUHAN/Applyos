...
import { applyBenchmarkShorthand, parseRunArgs, resolveRunOptions, withEnvOverrides, } from "../../tui/commands/parse.js";
describe("resolveRunOptions", () => {
    ...
    it("accepts explicit agent modes", () => {
        for (const agentMode of ["dom", "hybrid", "cua"]) {
    ...
    it("rejects unknown agent modes", () => {
        ...
        expect(() => parseRunArgs(["b:webvoyager", "--agent-modes", "dom,visual"])).toThrow(/agent-mode/);
    });
    it("rejects unknown bench harnesses", () => {
        expect(() => resolveRunOptions({ harness: "not_a_harness" }, {}, {})).toThrow(/Unknown harness/);
    });
...
describe("withEnvOverrides", () => {
    ...
    afterEach(() => {
        for (const key of [...stamped, "EVAL_ENV"])
