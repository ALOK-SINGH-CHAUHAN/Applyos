...
describe("readConfig", () => {
    it("throws on malformed evals.config.json instead of treating it as empty", () => {
        ...
        fs.writeFileSync(path.join(entryDir, "evals.config.json"), '{ "defaults": { "env": "local", } }');
...
describe("handleConfig", () => {
    it("persists agentModes defaults", async () => {
        const entryDir = makeTempEntryDir();
        fs.writeFileSync(path.join(entryDir, "evals.config.json"), JSON.stringify({ defaults: {}, benchmarks: {} }));
