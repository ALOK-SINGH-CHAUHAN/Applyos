...
import { canExecuteBenchHarness, deriveCategoryFilter, runCommand, } from "../../tui/commands/run.js";
const runEvalsMock = vi.hoisted(() => vi.fn(async () => ({
...
describe("deriveCategoryFilter", () => {
    ...
    it("expands dry-run matrices across configured agent modes", async () => {
        ...
        expect(payload.matrix.map((row) => row.agentMode)).toEqual(["dom", "hybrid"]);
        expect(payload.matrix.map((row) => row.harnessConfig)).toEqual([
    ...
    it("rejects claude_code for unsupported bench targets instead of emitting an empty matrix", async () => {
        ...
        await expect(runCommand({
            ...
            successMode: "outcome",
            verbose: false,
        }, registry)).rejects.toThrow(/only supports agent benchmark suites/);
    });
    it("rejects --api for non-stagehand bench harnesses even in dry-run", async () => {
        ...
        await expect(runCommand({
            ...
            successMode: "outcome",
            verbose: false,
        }, registry)).rejects.toThrow(/does not support --api/);
    });
...
describe("buildCombinations (preview column-pruning)", () => {
    ...
    it("surfaces model and agentMode for an agent matrix", async () => {
        ...
        const matrix = tasks.flatMap((task) => models.flatMap((model) => modes.map((agentMode) => ({
            tier: "bench",
            task,
            category: null,
            dataset: null,
