...
vi.mock("../../lib/braintrust-report.js", () => ({
    resolveExperimentProjectsAcrossProjects: async (_projects, inputs) => inputs.map((input, index) => ({
...
function makeChildProcess(args) {
...
describe("experiments compare", () => {
    ...
    it("passes the headless default output path to the renderer", async () => {
        spawnMock.mockImplementation((_command, args) => makeChildProcess(args));
