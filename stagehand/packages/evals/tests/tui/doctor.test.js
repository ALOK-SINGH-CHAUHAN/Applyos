...
function makeTempEntryDir(defaults) {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "evals-doctor-"));
    tempDirs.push(dir);
    fs.writeFileSync(path.join(dir, "evals.config.json"), JSON.stringify({ defaults: defaults ?? { env: "local", trials: 3 }, benchmarks: {} }, null, 2));
    return dir;
}
async function runDoctorJson(entryDir) {
    const chunks = [];
    const spy = vi
        .spyOn(console, "log")
    ...
    return { exit, report: JSON.parse(out) };
}
