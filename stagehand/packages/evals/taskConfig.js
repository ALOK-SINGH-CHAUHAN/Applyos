 ...
 */
function buildTasksConfigFromFS() {
    const configs = [];
...
const tasksByName = tasksConfig.reduce((acc, task) => {
    acc[task.name] = {
 ...
 */
export function validateEvalName(evalName) {
...
const isCuaModel = (modelName) => AVAILABLE_CUA_MODELS.includes(modelName);
...
function hasProviderEnvSupport(modelName) {
    try {
...
function uniqueAgentEntries(entries) {
    const seen = new Set();
...
function buildAgentModelEntries() {
    return uniqueAgentEntries([
        ...getConfiguredAgentModels().flatMap((modelName) => DEFAULT_AGENT_MODEL_MODES.map((mode) => ({
            modelName,
            mode,
            cua: false,
        }))),
...
const filterModelByProvider = (model, provider) => {
    ...
    if (provider === "openai") {
        ...
        return (modelLower.startsWith("meta-llama") ||
...
const MODELS = getModelList().map((model) => {
    return model;
});
const getAgentModelEntries = () => buildAgentModelEntries();
