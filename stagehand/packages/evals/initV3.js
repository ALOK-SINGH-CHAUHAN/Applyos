...
export async function initV3({ llmClient, modelClientOptions, logger, configOverrides, modelName, createAgent, agentMode, isCUA, verbose = false, }) {
    // If CUA, choose a safe internal AISDK model for V3 handlers based on available API keys
    let internalModel = modelName;
    const resolvedAgentMode = agentMode ?? (isCUA ? "cua" : undefined);
    const isCuaMode = resolvedAgentMode === "cua";
    if (isCuaMode) {
        if (process.env.OPENAI_API_KEY)
            internalModel = "openai/gpt-4.1-mini";
        else if (process.env.GEMINI_API_KEY ||
            process.env.GOOGLE_GENERATIVE_AI_API_KEY)
            internalModel = "google/gemini-2.0-flash";
        else if (process.env.ANTHROPIC_API_KEY)
            internalModel = "anthropic/claude-sonnet-4-6";
        else
            throw new Error("V3 init: No AISDK API key found. Set one of OPENAI_API_KEY, GEMINI_API_KEY/GOOGLE_GENERATIVE_AI_API_KEY, or ANTHROPIC_API_KEY to run CUA evals.");
    }
    const resolvedModelConfig = !isCuaMode && modelClientOptions
        ? {
            ...modelClientOptions,
    ...
    const v3Options = {
        ...
        localBrowserLaunchOptions: {
            ...
            args: configOverrides?.localBrowserLaunchOptions?.args ??
                configOverrides?.chromeFlags,
        },
        model: resolvedModelConfig,
        experimental: typeof configOverrides?.experimental === "boolean"
