 ...
 */
export function compareStrings(actual, expected, similarityThreshold = 0.85) {
 ...
 */
export function generateExperimentName({ evalName, category, }) {
    if (evalName)
        return evalName;
    if (category)
        return category;
...
function clipLogLine(line) {
    ...
    const maxWidth = typeof terminalWidth === "number" && terminalWidth > 8
        ? terminalWidth - 1
        : 119;
...
export function logLineToString(logLine) {
    try {
        ...
        if (logLine.auxiliary?.error) {
            ...
            return clipLogOutput(`${timestamp}::[stagehand:${logLine.category}] ${logLine.message}\n ${errorValue}${traceSuffix}`);
        }
        return clipLogOutput(`${timestamp}::[stagehand:${logLine.category}] ${logLine.message} ${logLine.auxiliary ? JSON.stringify(logLine.auxiliary) : ""}`);
    }
    catch (error) {
...
export function sampleUniform(arr, k) {
    const n = arr.length;
    if (k >= n)
        return arr.slice();
...
export function readJsonlFile(filePath) {
    ...
    try {
        ...
        console.warn(`Could not read file at ${filePath}. Error: ${e instanceof Error ? e.message : String(e)}`);
        lines = [];
    }
    return lines;
}
export function parseJsonlRows(lines, validator) {
    const candidates = [];
...
export function applySampling(candidates, sampleCount, maxCases = 25) {
    if (sampleCount && sampleCount > 0) {
        ...
        for (const candidate of candidates) {
            ...
            if (result.length >= maxCases)
                break;
        }
        return result;
    }
}
export function normalizeAgentModelEntries(models) {
    if (models.length === 0)
        return [];
