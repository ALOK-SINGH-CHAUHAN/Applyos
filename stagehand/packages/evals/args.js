...
function buildUsage(detailed = false) {
...
if (!filterByEvalName && parsedArgs.leftover.length > 0) {
    if (parsedArgs.leftover[0].toLowerCase() === "category") {
        ...
        try {
            ...
            console.error(chalk.red(`Error: Invalid category "${filterByCategory}". Valid categories are: ${DEFAULT_EVAL_CATEGORIES.join(", ")}`));
            process.exit(1);
        }
    }
    else {
