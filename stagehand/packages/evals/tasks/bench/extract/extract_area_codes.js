...
import { z } from "zod";
export default defineBenchTask({ name: "extract_area_codes" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/ncc-area-codes/", { waitUntil: "domcontentloaded" });
        const result = await v3.extract("Extract ALL the Primary Center names and their corresponding Area Code, and the name of their corresponding Zone.", z.object({
            primary_center_list: z.array(z.object({
                zone_name: z
                    .string()
                    .describe("The name of the Zone that the Primary Center is in. For example, 'North Central Zone'."),
                primary_center_name: z
                    .string()
                    .describe("The name of the Primary Center. I.e., this is the name of the city or town."),
                area_code: z
                    .string()
        ...
        const firstItemMatches = primaryCenterList[0].zone_name === expectedFirstItem.zone_name &&
        ...
        const lastItemMatches = primaryCenterList[primaryCenterList.length - 1].zone_name ===
