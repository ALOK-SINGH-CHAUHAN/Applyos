...
import { z } from "zod";
export default defineBenchTask({ name: "extract_csa" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/csa/");
        const result = await v3.extract("Extract all the publications on the page including the publication date, session type, publication type, and annotation", z.object({
        ...
        const expectedFirstItem = {
            ...
            annotation: "2024 -- This publication includes the complete histories of second-year bills. The complete electronic history of all bills is always available at leginfo.legislature.ca.gov",
        };
        ...
        const hasExpectedFirstItem = publications.some((publication) => {
            return (publication.publication_date === expectedFirstItem.publication_date &&
        ...
        const hasExpectedLastItem = publications.some((publication) => {
            return (publication.publication_date === expectedLastItem.publication_date &&
