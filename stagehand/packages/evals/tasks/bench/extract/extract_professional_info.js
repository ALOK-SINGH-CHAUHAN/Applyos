...
import { z } from "zod";
export default defineBenchTask({ name: "extract_professional_info" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/professional-info/");
        const result = await v3.extract("Extract the list of Practices, phone number, and fax number of the professional.", z.object({
        ...
        if (JSON.stringify(practices.map(normalizeString)) !==
