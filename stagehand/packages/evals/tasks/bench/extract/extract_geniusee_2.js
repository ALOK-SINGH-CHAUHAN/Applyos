...
export default defineBenchTask({ name: "extract_geniusee_2" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        ...
        const scalability = await v3.extract("Extract the scalability comment in the table for Gemini (Google)", z.object({
        ...
        if (commentMatches) {
            logger.error({
                message: "extracted scalability comment matches the scalability comment that we SHOULD NOT get",
                level: 0,
                auxiliary: {
                    expected: {
