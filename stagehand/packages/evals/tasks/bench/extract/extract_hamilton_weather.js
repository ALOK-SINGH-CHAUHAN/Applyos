...
export default defineBenchTask({ name: "extract_hamilton_weather" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        ...
        const weatherData = await v3.extract("extract the weather data for Sun, Feb 23 at 11PM", z.object({
        ...
        const isWeatherCorrect = compareStrings(weatherData.temperature, expectedWeatherData.temperature, 0.9).meetsThreshold &&
            compareStrings(weatherData.weather_description, expectedWeatherData.weather_description, 0.9).meetsThreshold &&
