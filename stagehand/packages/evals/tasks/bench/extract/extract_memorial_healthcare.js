...
export default defineBenchTask({ name: "extract_memorial_healthcare" }, async ({ logger, debugUrl, sessionUrl, v3, }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/mycmh/");
        const result = await v3.extract("extract a list of the first three healthcare centers on this page, with their name, full address, and phone number", z.object({
            health_centers: z.array(z.object({
                name: z.string(),
        ...
        const validateHealthCenter = (center) => {
        ...
        const compareField = (actual, expected, fieldName) => {
            const { similarity, meetsThreshold } = compareStrings(actual, expected, similarityThreshold);
            if (!meetsThreshold) {
                logger.error({
        ...
        const compareItem = (actual, expected, position) => {
            const fields = [
            ...
            return fields.every(({ field, actual, expected }) => compareField(actual, expected, `${position} ${field}`));
        };
