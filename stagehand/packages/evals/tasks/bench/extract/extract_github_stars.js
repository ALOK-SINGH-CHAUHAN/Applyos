...
import { z } from "zod";
export default defineBenchTask({ name: "extract_github_stars" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://github.com/facebook/react");
        const { stars } = await v3.extract("Extract the number of stars for the project", z.object({
