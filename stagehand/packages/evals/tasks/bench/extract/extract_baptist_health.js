...
import { z } from "zod";
export default defineBenchTask({ name: "extract_baptist_health" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/baptist-health/");
        const result = await v3.extract("Extract the address, phone number, and fax number of the healthcare location.", z.object({
            address: z.string(),
            phone: z.string(),
            fax: z.string(),
        }));
        ...
        const failedFields = [];
        const compareField = (actualVal, expectedVal, fieldName) => {
