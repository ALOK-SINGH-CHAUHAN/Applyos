...
import { z } from "zod";
export default defineBenchTask({ name: "extract_jfk_links" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/jfk/");
        const extraction = await v3.extract("extract all the record file name and their corresponding links", z.object({
        ...
        const missingRecords = expectedRecords.filter((expected) => {
            return !extractedRecords.some((r) => r.file_name === expected.file_name && r.link === expected.link);
        });
