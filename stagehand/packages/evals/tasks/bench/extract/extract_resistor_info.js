...
import { z } from "zod";
export default defineBenchTask({ name: "extract_resistor_info" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/resistor/");
        const result = await v3.extract("Extract the manufacturer standard lead time, tolerance percentage, resistance, and operating temperature range of the resistor.", z.object({
            manufacturer_standard_lead_time: z.string(),
            tolerance_percentage: z.string(),
            resistance: z.string(),
            operating_temperature_range: z.string(),
        }));
        const { manufacturer_standard_lead_time, tolerance_percentage, resistance, operating_temperature_range, } = result;
        const expected = {
        ...
        if (normalizeString(manufacturer_standard_lead_time) !==
        ...
        if (normalizeString(tolerance_percentage) !==
