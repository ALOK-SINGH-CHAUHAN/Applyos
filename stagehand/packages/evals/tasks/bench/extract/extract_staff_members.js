...
export default defineBenchTask({ name: "extract_staff_members" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://browserbase.github.io/stagehand-eval-sites/sites/panamcs/");
        const result = await v3.extract("extract a list of ALL the staff members on this page, with their name and their job title", z.object({
            staff_members: z.array(z.object({
                name: z.string(),
                job_title: z.string(),
            })),
        }));
        ...
        const firstItemExists = staff_members.some((member) => member.name === expectedFirstItem.name &&
        ...
        const lastItemExists = staff_members.some((member) => member.name === expectedLastItem.name &&
