...
{ length: 2500 }, (_, index) => `<span hidden data-filler="${index}">filler ${index}</span>`).join("");
const cases = [
    {
...
export default defineBenchTask({ name: "observe_main_frame_element_ids" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
