...
export default defineBenchTask({ name: "extract_snowshoeing_destinations" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        const page = v3.context.pages()[0];
        await page.goto("https://www.cbisland.com/blog/10-snowshoeing-adventures-on-cape-breton-island/");
        await v3.act("accept the cookies");
        const snowshoeing_regions = await v3.extract("Extract all the snowshoeing regions and the names of the trails within each region.", z.object({
            snowshoeing_regions: z.array(z.object({
                region_name: z
                    .string()
