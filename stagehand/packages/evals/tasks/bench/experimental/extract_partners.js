...
import { z } from "zod";
export default defineBenchTask({ name: "extract_partners" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const partners = await v3.extract(`Extract all of the partner categories on the page.`, z.object({
        ...
        const foundPartners = partners.partners.map((partner) => partner.partner_category.toLowerCase());
