...
import { z } from "zod";
export default defineBenchTask({ name: "ted_talk" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const playlists = await v3.extract("Extract the video playlist titles and the number of talks in each playlist. This info is in the Video Playlists about Culture section of the webpage.", z.object({
            playlists: z
                .array(z.object({
        ...
        const missingPlaylists = expectedPlaylists.filter((expected) => playlists.playlists.every((extracted) => normalizeString(extracted.title) !==
