...
export default defineBenchTask({ name: "agent/ubereats" }, async ({ debugUrl, sessionUrl, logger, agent, v3 }) => {
    try {
        const initUrl = "https://www.ubereats.com/";
        const page = v3.context.pages()[0];
        await page.goto(initUrl);
        const instruction = "Order a pizza from ubereats to 639 geary st in sf, call the task complete once the login page is shown after adding pizza and viewing the cart";
        const taskSpec = {
            id: "agent/ubereats",
            instruction,
            initUrl,
