...
export default defineBenchTask({ name: "agent/radiotimes_tv_schedule" }, async ({ debugUrl, sessionUrl, logger, agent, v3 }) => {
    try {
        const initUrl = "https://radiotimes.com";
        const page = v3.context.pages()[0];
        await page.goto(initUrl);
        const instruction = "Locate tonight's featured TV schedule on Radiotimes, and list the titles of shows airing on both BBC and ITV. Only use http://radiotimes.com to achieve the task. Don't go to any other site. The task is achievable with just navigation from this site.";
        const taskSpec = {
