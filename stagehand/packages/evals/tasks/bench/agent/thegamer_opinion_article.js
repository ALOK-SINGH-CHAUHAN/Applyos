...
export default defineBenchTask({ name: "agent/thegamer_opinion_article" }, async ({ debugUrl, sessionUrl, logger, agent, v3 }) => {
    try {
        const initUrl = "https://www.thegamer.com";
        const page = v3.context.pages()[0];
        await page.goto(initUrl);
        const instruction = "Locate an Opinion or Cultural Commentary article discussing modern gaming culture and summarize its central argument in one or two sentences. Only use http://thegamer.com to achieve the task. Don't go to any other site. The task is achievable with just navigation from this site.";
        const taskSpec = {
            id: "agent/thegamer_opinion_article",
            instruction,
            initUrl,
            precomputedRubric: adHocRubric(`did the agent complete this task successfully? ${instruction}`),
        };
