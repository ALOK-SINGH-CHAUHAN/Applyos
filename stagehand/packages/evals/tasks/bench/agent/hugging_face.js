...
export default defineBenchTask({ name: "agent/hugging_face" }, async ({ debugUrl, sessionUrl, logger, agent, v3 }) => {
    try {
        const initUrl = "https://huggingface.co/";
        const page = v3.context.pages()[0];
        await page.goto(initUrl);
        const instruction = "Search for a model on Hugging Face with an Apache-2.0 license that has received the highest number of likes.";
        const taskSpec = {
            id: "agent/hugging_face",
            instruction,
            initUrl,
            precomputedRubric: adHocRubric("Does the message mention 'kokoro-82m' or 'hexgrad/Kokoro-82M'?"),
        };
