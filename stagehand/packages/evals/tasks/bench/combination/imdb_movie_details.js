...
import { z } from "zod";
export default defineBenchTask({ name: "imdb_movie_details" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const movieDetails = await v3.extract("Extract the list of countries with the most ratings.", z.object({
            countries: z
                .array(z.string())
        ...
        const missingCountries = expectedCountries.filter((country) => !movieDetails.countries.includes(country));
