...
import { z } from "zod";
export default defineBenchTask({ name: "wichita" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const result = await v3.extract("Extract the total number of bids that the search produced.", z.object({
