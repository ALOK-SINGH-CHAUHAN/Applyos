...
export default defineBenchTask({ name: "history" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const allEntriesHaveTimestamps = history.every((entry) => typeof entry.timestamp === "string" && entry.timestamp.length > 0);
        const allEntriesHaveResults = history.every((entry) => entry.result !== undefined);
        const success = hasCorrectNumberOfEntries &&
            hasNavigateEntry &&
            hasActEntry &&
            hasExtractEntry &&
            hasObserveEntry &&
