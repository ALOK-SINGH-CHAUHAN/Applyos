...
import { z } from "zod";
export default defineBenchTask({ name: "peeler_complex" }, async ({ debugUrl, sessionUrl, v3, logger }) => {
    try {
        ...
        const { price } = await v3.extract("get the price of the peeler", z.object({ price: z.number().nullable() }));
        return {
