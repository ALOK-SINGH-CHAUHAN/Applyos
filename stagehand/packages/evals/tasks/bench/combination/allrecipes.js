...
import { z } from "zod";
export default defineBenchTask({ name: "allrecipes" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        ...
        const recipeDetails = await v3.extract("Extract the title of the first recipe and the total number of ratings it has received.", z.object({
        ...
        const isRatingsWithinRange = extractedRatings >= expectedRatings - 1000 &&
