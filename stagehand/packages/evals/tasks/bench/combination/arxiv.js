...
export default defineBenchTask({ name: "arxiv" }, async ({ logger, debugUrl, sessionUrl, v3 }) => {
    try {
        ...
        const paper_links = await v3.extract("extract the titles and links for two papers", z.object({
            papers: z
                .array(z.object({
        ...
        for (const paper of paper_links.papers) {
            if (paper.link) {
                ...
                const abstract = await v3.extract("extract details of the paper from the abstract", z.object({
                    category: z
                        .string()
