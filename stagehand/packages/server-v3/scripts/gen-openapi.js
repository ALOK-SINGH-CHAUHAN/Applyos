...
import { fastifyZodOpenApiPlugin, fastifyZodOpenApiTransformers, serializerCompiler, validatorCompiler, } from "fastify-zod-openapi";
...
function normalizeGeneratedOpenApiYaml(yaml) {
    ...
    return yaml.replace(/ {4}AzureModelConfigObject:\n(?:(?: {6}.*)\n)*(?= {4}ModelConfigObject:)/, renderAzureModelConfigSchema());
}
async function main() {
    const app = fastify({
        logger: false,
