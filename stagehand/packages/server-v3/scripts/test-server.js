...
const toSafeName = (name) => name.replace(/[\\/]/g, "-");
...
const writeCtrfFromJunit = (junitPath, tool) => {
    ...
    const ctrfPath = junitPath.match(/\.xml$/i)
        ...
        : `${junitPath}.json`;
    const result = spawnSync("pnpm", ["exec", "junit-to-ctrf", junitPath, "-o", ctrfPath, "-t", tool], { stdio: "inherit", cwd: repoRoot });
    if (result.status !== 0) {
...
const resolveRepoRelative = (value) => path.isAbsolute(value) ? value : path.resolve(repoRoot, value);
const stripNodeReporterArgs = (argsList) => {
    const filtered = [];
    let removed = false;
...
const toTestName = (testPath, root) => {
...
if (listRequested) {
    ...
    const integrationTests = collectFiles(sourceIntegrationDir, ".test.ts").map((file) => {
...
const { filtered: extraArgs, removed: removedReporterOverride } = stripNodeReporterArgs(extra);
if (removedReporterOverride) {
    console.warn("Ignoring node --test reporter overrides to preserve console + JUnit output.");
}
if (!fs.existsSync(allTestsDir)) {
    console.error("Missing packages/server-v3/dist/tests. Run pnpm run build first.");
    process.exit(1);
}
const serverTarget = (process.env.STAGEHAND_SERVER_TARGET ?? "sea").toLowerCase();
const explicitBaseUrl = process.env.STAGEHAND_API_URL ?? process.env.STAGEHAND_BASE_URL;
...
if (serverTarget === "remote" && !explicitBaseUrl) {
    console.error("Missing STAGEHAND_API_URL for remote server target. STAGEHAND_BASE_URL is accepted as a legacy fallback.");
    process.exit(1);
}
if (serverTarget === "local" &&
    !fs.existsSync(`${repoRoot}/packages/server-v3/dist/server.js`)) {
    console.error("Missing packages/server-v3/dist/server.js. Run pnpm run build first.");
    process.exit(1);
}
...
const unitPaths = allPaths.filter((p) => p.replaceAll("\\", "/").includes("/packages/server-v3/dist/tests/unit/"));
const integrationPaths = allPaths.filter((p) => p
    .replaceAll("\\", "/")
    .includes("/packages/server-v3/dist/tests/integration/"));
const singlePath = allPaths.length === 1 ? allPaths[0] : null;
const coverageSuffix = singlePath &&
    singlePath.startsWith(`${repoRoot}/packages/server-v3/dist/tests/unit/`)
    ? `server-unit/${path.basename(singlePath).replace(/\.test\.js$/, "")}`
    : singlePath &&
        singlePath.startsWith(`${repoRoot}/packages/server-v3/dist/tests/integration/`)
        ? `server-integration/${path
            .relative(integrationDir, singlePath)
            .replace(/\.test\.js$/, "")
            .replaceAll("\\", "/")}`
        : "server";
const coverageRoot = resolveRepoRelative(process.env.NODE_V8_COVERAGE ?? `${repoRoot}/coverage/${coverageSuffix}`);
...
const envDestination = process.env.NODE_TEST_REPORTER_DESTINATION
    ...
    : null;
const reporterArgsFor = (kind, testName) => {
    const destination = envDestination ??
...
const runNodeTests = (files, reporterArgs) => spawnSync(process.execPath, ["--test", ...extraArgs, ...reporterArgs, ...files], {
    stdio: "inherit",
    env: {
        ...process.env,
...
const startServer = async () => {
    if (serverTarget === "remote")
        return null;
    if (serverTarget === "local") {
        return spawn(process.execPath, [`${repoRoot}/packages/server-v3/dist/server.js`], {
            stdio: "inherit",
            env: {
                ...process.env,
...
if (unitPaths.length > 0) {
    const unitName = unitPaths.length === 1 ? toTestName(unitPaths[0], unitDir) : undefined;
...
if (status === 0 && integrationPaths.length > 0) {
    ...
    if (!ready) {
        ...
        const integrationName = integrationPaths.length === 1
