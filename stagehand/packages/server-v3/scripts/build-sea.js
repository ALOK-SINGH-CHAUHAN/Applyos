...
const parseBoolean = (value, fallback) => {
    ...
    throw new Error(`Invalid boolean value "${value}" for --include-sourcemaps / SEA_INCLUDE_SOURCEMAPS`);
};
const targetPlatform = argValue("target-platform") ??
    argValue("platform") ??
    process.env.SEA_TARGET_PLATFORM ??
    process.platform;
const targetArch = argValue("target-arch") ??
    argValue("arch") ??
    process.env.SEA_TARGET_ARCH ??
    process.arch;
const binaryName = argValue("binary-name") ??
    process.env.SEA_BINARY_NAME ??
    `stagehand-server-v3-${targetPlatform}-${targetArch}${targetPlatform === "win32" ? ".exe" : ""}`;
const includeSourcemaps = parseBoolean(argValue("include-sourcemaps") ?? process.env.SEA_INCLUDE_SOURCEMAPS, false);
const gitOutput = (args) => execFileSync("git", args, { cwd: repoDir, encoding: "utf8" }).trim();
...
const runNodeScript = (scriptPath, args, opts = {}) => run(process.execPath, [scriptPath, ...args], opts);
...
const runOptional = (cmd, args, opts = {}) => {
    spawnSync(cmd, args, { stdio: "ignore", ...opts });
};
const hasSeaFuse = (binaryPath) => {
    try {
...
const resolveNodeBinary = async () => {
    if (targetPlatform !== process.platform) {
        throw new Error(`Cross-platform builds are not supported. Host=${process.platform}, target=${targetPlatform}`);
    }
    if (targetArch === process.arch && hasSeaFuse(process.execPath)) {
        return process.execPath;
    }
    if (targetArch === process.arch) {
        console.warn(`Current Node binary at ${process.execPath} does not include ${seaFuse}; falling back to the official ${process.version} distribution for SEA injection.`);
    }
    ...
    const binaryPath = distPlatform === "win"
        ? `${extractRoot}/node.exe`
        : `${extractRoot}/bin/node`;
    if (fs.existsSync(binaryPath)) {
        if (!hasSeaFuse(binaryPath)) {
            throw new Error(`Node binary at ${binaryPath} does not include ${seaFuse}; unable to build SEA binary. Delete ${tmpRoot} and retry.`);
        }
        return binaryPath;
    }
...
const writeSeaConfig = (mainPath, outputPath, execArgvExtension) => {
...
const buildCjsBundle = () => {
    ...
    runNodeScript(turboBin, ["run", "build:cjs", "--filter", "@browserbasehq/stagehand"], {
        cwd: repoDir,
    });
...
const buildEsmBundle = () => {
    if (!fs.existsSync(`${repoDir}/packages/core/dist/esm/index.js`)) {
        throw new Error(`Missing ${repoDir}/packages/core/dist/esm/index.js. Run pnpm run build:esm first.`);
    }
    fs.mkdirSync(`${repoDir}/packages/server-v3/dist/sea`, { recursive: true });
    ...
    if (includeSourcemaps) {
        const mapMatch = appSource.match(/sourceMappingURL=data:application\/json;base64,([A-Za-z0-9+/=]+)\s*$/);
        if (!mapMatch) {
        ...
        const toRepoRelative = (source) => {
            ...
            const resolved = toPosix(path.resolve(`${repoDir}/packages/server-v3`, sourcePath));
...
const main = async () => {
    ...
    const seaConfigPath = writeSeaConfig(mainPath, `${repoDir}/packages/server-v3/dist/sea/sea-prep.blob`, execArgvExtension);
    ...
    if (!fs.existsSync(`${repoDir}/packages/server-v3/dist/sea/sea-prep.blob`)) {
        throw new Error(`Missing ${repoDir}/packages/server-v3/dist/sea/sea-prep.blob; SEA blob generation failed.`);
    }
