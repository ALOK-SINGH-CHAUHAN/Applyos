...
import { fastifyZodOpenApiPlugin, fastifyZodOpenApiTransformers, serializerCompiler, validatorCompiler, RequestValidationError, ResponseSerializationError, } from "fastify-zod-openapi";
import { StatusCodes } from "http-status-codes";
import { logging } from "./lib/logging/index.js";
import { getListenHostConfig } from "./lib/listenHost.js";
import { destroySessionStore, initializeSessionStore, } from "./lib/sessionStoreManager.js";
...
process.on("unhandledRejection", (reason, promise) => {
    app.log.error(reason instanceof Error ? reason : new Error(String(reason)), "Unhandled Rejection at:", promise, "reason:", reason);
});
...
const start = async () => {
    try {
        ...
        app.setErrorHandler((error, request, reply) => {
            if (error.validation) {
                const zodIssues = error.validation
                    .filter((err) => err instanceof RequestValidationError)
                    .map((err) => err.params.issue);
            ...
            reply.status(statusCode).send({
                error: statusCode === Number(StatusCodes.INTERNAL_SERVER_ERROR)
        ...
        const appWithTypes = app.withTypeProvider();
        await appWithTypes.register((instance, _opts, done) => {
