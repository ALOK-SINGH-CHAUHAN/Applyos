 ...
 */
function resolveRefs(schema, defs, seen = new Set()) {
    if (schema.$ref) {
    ...
    if (resolved.anyOf) {
        resolved.anyOf = resolved.anyOf.map((s) => resolveRefs(s, defs, new Set(seen)));
    }
    if (resolved.oneOf) {
        resolved.oneOf = resolved.oneOf.map((s) => resolveRefs(s, defs, new Set(seen)));
    }
    if (resolved.allOf) {
        resolved.allOf = resolved.allOf.map((s) => resolveRefs(s, defs, new Set(seen)));
    }
...
export function jsonSchemaToZod(schema) {
    ...
    const resolved = schema.$defs || schema.$ref
        ? resolveRefs(schema, schema.$defs ?? {})
        : schema;
    return _jsonSchemaToZod(resolved);
}
function _jsonSchemaToZod(schema) {
        ...
        case "object":
            if (schema.properties) {
                ...
                for (const key in schema.properties) {
                    ...
                    if (!subSchema) {
                        throw new AppError(`Property ${key} is not defined in the schema`, StatusCodes.BAD_REQUEST);
                    }
                    shape[key] = _jsonSchemaToZod(subSchema);
                }
                let zodObject = z.object(shape);
                if (schema.required && Array.isArray(schema.required)) {
                    const requiredFields = schema.required.reduce((acc, key) => {
                        acc[key] = true;
                        return acc;
                    }, {});
...
export function mapModelToProvider(model) {
    switch (model) {
        ...
        case "cerebras-llama-3.1-8b":
            throw new AppError("Cerebras models are not supported yet", StatusCodes.BAD_REQUEST);
        case "groq-llama-3.3-70b-specdec":
        case "groq-llama-3.3-70b-versatile":
            throw new AppError("Groq models are not supported yet", StatusCodes.BAD_REQUEST);
        default: {
