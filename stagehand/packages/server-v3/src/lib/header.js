...
export const dangerouslyGetHeader = (request, header) => {
...
export const getOptionalHeader = (request, header) => {
    const headerValue = request.headers[header];
    if (!headerValue) {
        return undefined;
    }
...
const requestModelInputSchema = z
    .union([
    emptyStringSchema,
    requiredStringSchema.transform((modelName) => ({ modelName })),
 ...
 */
export function getRequestModelConfig(request) {
 ...
 */
export function getStagehandInitModelConfig(request, requestModelConfig) {
    ...
    const bodyResult = stagehandInitModelEnvelopeSchema.safeParse(request.body ?? {});
    if (!bodyResult.success) {
        return bodyResult;
    }
 ...
 */
export function shouldRespondWithSSE(request) {
