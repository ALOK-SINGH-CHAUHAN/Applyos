...
import { assertFetchOk, assertFetchStatus, createSessionWithCdp, endSession, fetchWithContext, getBaseUrl, getHeaders, HTTP_BAD_REQUEST, HTTP_OK, } from "../utils.js";
// =============================================================================
...
describe("POST /v1/sessions/:id/navigate (V3)", () => {
    ...
    it("should navigate to a URL successfully and verify via CDP", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/navigate`, {
            method: "POST",
        ...
        assertFetchOk(typeof ctx.body.data.actionId === "string", "Response should have actionId", ctx);
        ...
        assert.ok(pageUrl.includes("example.com"), `Page URL should be example.com, got: ${pageUrl}`);
        await browser.close();
    });
    ...
    it("should return 400 when url is missing", async () => {
        ...
        assertFetchOk(!ctx.body.success ||
            ctx.body.error !== undefined ||
            ctx.body.message !== undefined, "Response should indicate failure", ctx);
    });
});
