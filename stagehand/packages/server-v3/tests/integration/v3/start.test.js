...
import { assertFetchOk, assertFetchStatus, endSession, fetchWithContext, getBaseUrl, getHeaders, LOCAL_BROWSER_BODY, HTTP_BAD_REQUEST, HTTP_OK, } from "../utils.js";
...
afterEach(async () => {
    await Promise.all([...activeSea].map(async (handle) => {
...
function parseSupervisorConfigArg(args) {
...
function findLocalSupervisorByParentPid(parentPid) {
    const candidates = listProcesses()
        .map((entry) => ({
        ...entry,
        config: parseSupervisorConfigArg(entry.args),
    }))
        .filter((entry) => entry.config?.kind === "LOCAL" && entry.config.parentPid === parentPid)
    ...
    return {
        pid: entry.pid,
        args: entry.args,
        chromePid: typeof entry.config?.pid === "number" && Number.isFinite(entry.config.pid)
            ? entry.config.pid
            : undefined,
    };
}
...
async function waitForValue(read, timeoutMs, intervalMs = 200) {
...
async function waitForPidState(pid, shouldBeAlive, timeoutMs) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeoutMs) {
        if (isPidAlive(pid) === shouldBeAlive)
            return;
        await sleep(200);
    }
    const entry = listProcesses().find((candidate) => candidate.pid === pid);
    const details = entry ? ` args=${entry.args}` : "";
    throw new Error(`PID ${pid} did not become ${shouldBeAlive ? "alive" : "dead"} within ${timeoutMs}ms${details}`);
}
...
async function startSeaServer(envOverrides = {}) {
    const port = await getFreePort();
    const baseUrl = `http://127.0.0.1:${port}`;
    const logs = [];
    const proc = spawn(seaBinaryPath, ["--node-options=--no-lazy --enable-source-maps"], {
        env: {
            ...process.env,
            ...envOverrides,
            NODE_ENV: "production",
            PORT: String(port),
            STAGEHAND_SEA_CACHE_DIR: process.env.STAGEHAND_SEA_CACHE_DIR ?? `${repoRoot}/.stagehand-sea`,
        },
    ...
    const handle = { proc, baseUrl, logs };
    ...
    try {
        ...
        throw new Error(`Failed to start SEA server at ${baseUrl}: ${error.message}\n${tail}`, {
            cause: error,
        });
    }
}
async function stopSeaServer(handle) {
    const { proc } = handle;
    if (proc.exitCode !== null)
        return;
    try {
...
async function forceKillSeaServer(handle) {
    const { proc } = handle;
    if (proc.exitCode !== null)
        return;
    try {
...
async function startKeepAliveFalseLocalSession(baseUrl) {
    const headers = getHeaders("3.0.0");
    const ctx = await fetchWithContext(`${baseUrl}/v1/sessions/start`, {
        method: "POST",
        headers,
        body: JSON.stringify({
    ...
    assert.equal(ctx.status, HTTP_OK, `Expected local /start to succeed, got ${ctx.status}\n${ctx.debugSummary()}`);
    assertFetchOk(ctx.body !== null, "Should have response body", ctx);
    assertFetchOk(isSuccessResponse(ctx.body), "Should return a successful start response", ctx);
    return {
        sessionId: ctx.body.data.sessionId,
        cdpUrl: ctx.body.data.cdpUrl,
    };
}
async function startKeepAliveFalseBrowserbaseSession(baseUrl) {
    ...
    assert.equal(ctx.status, HTTP_OK, `Expected browserbase /start to succeed, got ${ctx.status}\n${ctx.debugSummary()}`);
    assertFetchOk(ctx.body !== null, "Should have response body", ctx);
    assertFetchOk(isSuccessResponse(ctx.body), "Should return a successful start response", ctx);
    const sessionId = ctx.body.data.sessionId;
    // Browserbase Stagehand init is lazy; navigate once to ensure supervisor is running.
    const navigateCtx = await fetchWithContext(`${baseUrl}/v1/sessions/${sessionId}/navigate`, {
        method: "POST",
        headers,
        body: JSON.stringify({ url: "https://example.com", frameId: "" }),
    });
    assert.equal(navigateCtx.status, HTTP_OK, `Expected browserbase /navigate to succeed, got ${navigateCtx.status}\n${navigateCtx.debugSummary()}`);
    return sessionId;
}
async function closeLocalBrowserViaCdp(cdpUrl) {
...
async function waitForBrowserbaseNotRunning(sessionId, timeoutMs) {
    assert.ok(bbApiKey, "BROWSERBASE_API_KEY must be set");
    ...
    throw new Error(`Browserbase session ${sessionId} stayed RUNNING for ${timeoutMs}ms (last status=${lastStatus})`);
}
async function requestBrowserbaseReleaseBestEffort(sessionId) {
...
describe("POST /v1/sessions/start - V3 format", () => {
    ...
    it("should start session without x-model-api-key header", async () => {
        ...
        const headersWithoutModelKey = Object.fromEntries(Object.entries(headers).filter(([k]) => k !== "x-model-api-key"));
        const ctx = await fetchWithContext(`${url}/v1/sessions/start`, {
            method: "POST",
            headers: headersWithoutModelKey,
            body: JSON.stringify({ modelName: "gpt-4.1-nano", ...localBrowser }),
        });
        assertFetchStatus(ctx, HTTP_OK, "Session start without x-model-api-key should succeed");
        assertFetchOk(ctx.body !== null, "Should have response body", ctx);
        assertFetchOk(isSuccessResponse(ctx.body), "Should be a success response", ctx);
    ...
    it("should return cdpUrl as a valid WebSocket URL for local browser", async () => {
        ...
        assertFetchOk(ctx.body.data.cdpUrl !== "", "cdpUrl should not be empty", ctx);
        // cdpUrl should be a valid WebSocket URL
        assertFetchOk(ctx.body.data.cdpUrl.startsWith("ws://"), "cdpUrl should be a WebSocket URL", ctx);
        await endSession(ctx.body.data.sessionId, headers);
    });
    it("should return provided cdpUrl when explicit cdpUrl is passed", async () => {
        ...
        assertFetchOk(ctx.body !== null, "Should have response body", ctx);
        assertFetchOk(isSuccessResponse(ctx.body), "Should be a success response", ctx);
        assertFetchOk(ctx.body.data.cdpUrl === providedCdpUrl, "cdpUrl should match provided value", ctx);
        await endSession(ctx.body.data.sessionId, headers);
    });
    ...
    it("should start browserbase session with API key but no project ID", async () => {
        if (!bbApiKey)
            return; // skip when credentials unavailable
        const url = getBaseUrl();
        const bbHeaders = {
            ...getHeaders("3.0.0"),
            "x-bb-api-key": bbApiKey,
    ...
    it("should accept non-default region in browserbaseSessionCreateParams", async () => {
        ...
        assertFetchOk(ctx.body.data.available === true, "Session should be available for non-default regions", ctx);
        assertFetchOk(!!ctx.body.data.sessionId, "Should have sessionId", ctx);
        await endSession(ctx.body.data.sessionId, headers);
    });
    it("should accept eu-central-1 region in browserbaseSessionCreateParams", async () => {
        ...
        assertFetchOk(ctx.body !== null, "Should have response body", ctx);
        assertFetchOk(isSuccessResponse(ctx.body), "Should be a success response", ctx);
        assertFetchOk(ctx.body.data.available === true, "Session should be available for eu-central-1 region", ctx);
        assertFetchOk(!!ctx.body.data.sessionId, "Should have sessionId", ctx);
        await endSession(ctx.body.data.sessionId, headers);
    });
    it("should accept ap-southeast-1 region in browserbaseSessionCreateParams", async () => {
        ...
        assertFetchOk(ctx.body !== null, "Should have response body", ctx);
        assertFetchOk(isSuccessResponse(ctx.body), "Should be a success response", ctx);
        assertFetchOk(ctx.body.data.available === true, "Session should be available for ap-southeast-1 region", ctx);
        assertFetchOk(!!ctx.body.data.sessionId, "Should have sessionId", ctx);
        await endSession(ctx.body.data.sessionId, headers);
    });
});
describe("POST /v1/sessions/start - keepAlive=false supervision in SEA", () => {
    it("spawns a supervisor and exits it when chrome dies", async () => {
        ...
        const supervisor = await waitForValue(() => findLocalSupervisorByParentPid(seaPid), 10_000);
        assert.ok(supervisor.chromePid, `Expected local supervisor to include --chrome-pid. args=${supervisor.args}`);
        assert.ok(isPidAlive(supervisor.pid), `Supervisor PID ${supervisor.pid} should be alive`);
        assert.ok(isPidAlive(supervisor.chromePid), `Chrome PID ${supervisor.chromePid} should be alive`);
        ...
        assert.ok(isPidAlive(seaPid), "SEA process should stay alive after chrome dies");
    });
    ...
    it("force-killing SEA ends Browserbase session when keepAlive=false", async () => {
        ...
        const sessionId = await startKeepAliveFalseBrowserbaseSession(handle.baseUrl);
        try {
            await forceKillSeaServer(handle);
            const finalStatus = await waitForBrowserbaseNotRunning(sessionId, 30_000);
            assert.notEqual(finalStatus, "RUNNING", "Browserbase session should not remain RUNNING after SEA kill");
        }
        finally {
