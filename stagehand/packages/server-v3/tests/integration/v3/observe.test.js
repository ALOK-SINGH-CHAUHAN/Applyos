...
import { assertEventExists, assertFetchOk, assertFetchStatus, assertWithContext, createSessionWithCdp, endSession, fetchWithContext, GEMINI_API_KEY, getBaseUrl, getHeaders, getMainFrameId, HTTP_OK, navigateSession, readTypedSSEStreamWithContext, requireEnv, } from "../utils.js";
...
describe("POST /v1/sessions/:id/observe (V3)", () => {
    it("should observe elements with instruction", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/observe`, {
            method: "POST",
        ...
        assertFetchOk(Array.isArray(ctx.body.data.result), "Result should be an array of observed elements", ctx);
    });
    ...
    it("should observe with variables option", async () => {
        const url = getBaseUrl();
        const navResponse = await navigateSession(sessionId, "https://browserbase.github.io/stagehand-eval-sites/sites/login/", getHeaders("3.0.0"));
        ...
        try {
            ...
            await navigateSession(sessionId, "https://example.com", getHeaders("3.0.0"));
        }
    });
    it("should observe with rich variables option", async () => {
        ...
        try {
            ...
            assertFetchStatus(ctx, HTTP_OK, "Observe with rich variables should succeed");
            assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
            assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
            assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
            assertFetchOk(Array.isArray(ctx.body.data.result), "Result should be an array of observed elements", ctx);
    ...
    it("should observe without instruction (observe all)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Observe without instruction should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(Array.isArray(ctx.body.data.result), "Result should be an array of observed elements", ctx);
    ...
    it("should observe with google/gemini-2.5-flash-lite model", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Observe with google/gemini-2.5-flash-lite model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(Array.isArray(ctx.body.data.result), "Result should be an array of observed elements", ctx);
...
describe("POST /v1/sessions/:id/observe with SSE streaming (V3)", () => {
    it("should stream valid SSE events with correct structure", async () => {
        ...
        assertWithContext(Array.isArray(finishedEvent.data.result), "Result must be an array of observed elements", ctx);
    });
    it("should have correct event sequence: starting -> connected -> finished", async () => {
        ...
        const connectedIndex = events.findIndex((e) => e.data.status === "connected");
    ...
    it("should return observed elements with expected properties", async () => {
        ...
        assertWithContext(Array.isArray(finishedEvent.data.result), "Result should be an array", ctx);
        // If there are observed elements, verify they have expected structure
        if (finishedEvent.data.result.length > 0) {
            const firstElement = finishedEvent.data.result[0];
            assertWithContext(typeof firstElement === "object", "Each observed element should be an object", ctx);
            // Observed elements typically have selector and description
            assertWithContext("selector" in firstElement || "description" in firstElement, "Observed element should have selector or description", ctx);
        }
    });
});
