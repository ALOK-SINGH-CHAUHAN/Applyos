...
import { assertFetchOk, assertFetchStatus, createSession, endSession, fetchWithContext, getBaseUrl, getHeaders, HTTP_BAD_REQUEST, HTTP_NOT_FOUND, HTTP_INTERNAL_SERVER_ERROR, HTTP_OK, } from "../utils.js";
// =============================================================================
...
describe("POST /v1/sessions/:id/end (V3)", () => {
    ...
    it("should return 200 if JSON content-type has an empty body", async () => {
        ...
        }, HTTP_OK, "Should return 200 for empty body with JSON content-type");
        // Clean up
    ...
    it("should return 400 if body contains extra keys", async () => {
        const url = getBaseUrl();
        const testSessionId = await createSession(headers);
        const ctx = await fetchWithContext(`${url}/v1/sessions/${testSessionId}/end`, {
            method: "POST",
            headers,
            body: JSON.stringify({ unexpected: true }),
        });
        assertFetchStatus(ctx, HTTP_BAD_REQUEST, "Should return 400 for extra keys");
        // Clean up
        await endSession(testSessionId, headers);
    });
    ...
    it("should return 200 when body is 0 bytes (no body)", async () => {
        ...
        assert.equal(response.status, HTTP_OK, `Should return 200 for 0-byte body, got ${response.status}`);
        const body = await response.json();
        assert.equal(body.success, true, "Should indicate success");
    });
    it("should end session successfully", async () => {
        const url = getBaseUrl();
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/end`, {
            method: "POST",
            headers,
            body: JSON.stringify({}),
        });
    ...
    it("should return error for non-existent session", async () => {
        const url = getBaseUrl();
        const ctx = await fetchWithContext(`${url}/v1/sessions/non-existent-session-id/end`, {
            method: "POST",
            headers,
            body: JSON.stringify({}),
        });
        // Server returns 404 or 500 for non-existent sessions
        assert.ok([HTTP_NOT_FOUND, HTTP_INTERNAL_SERVER_ERROR].includes(ctx.status), `Expected 404 or 500, got ${ctx.status}`);
        if (ctx.status === HTTP_INTERNAL_SERVER_ERROR) {
            assertFetchOk(ctx.body !== null, "Response should have body", ctx);
            assertFetchOk(ctx.body.message === "An internal server error occurred", "500 responses should return a generic internal error message", ctx);
        }
    });
});
