...
import { assertEventExists, assertFetchOk, assertFetchStatus, assertWithContext, createSessionWithCdp, endSession, fetchWithContext, GEMINI_API_KEY, getBaseUrl, getHeaders, getMainFrameId, HTTP_OK, navigateSession, OPENAI_API_KEY, readTypedSSEStreamWithContext, readSSEStream, requireEnv, } from "../utils.js";
...
beforeEach(async () => {
    const navResponse = await navigateSession(sessionId, "https://example.com", getHeaders("3.0.0"));
...
describe("POST /v1/sessions/:id/act (V3)", () => {
    ...
    it("should perform an action using string input format", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/act`, {
            method: "POST",
            headers: {
                ...getHeaders("3.0.0"),
            },
        ...
        assert.equal(typeof ctx.body.data.result.success, "boolean", "Result should have success boolean");
        ...
        assert.ok(pageUrl.includes("iana.org/help/example-domains"), `Page URL should be iana.org/help/example-domains, got: ${pageUrl}`);
        await browser.close();
    });
    ...
    it("should perform action with inline model config (modelName + apiKey)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "act with inline model config should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
    ...
    it("should perform action with inline model config and options", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "act with inline model config and options should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
    ...
    it("should perform action with object input and inline model config", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "act with object input and inline model config should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
    ...
    it("should perform action with google/gemini-2.5-flash-lite model", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "act with google/gemini-2.5-flash-lite model should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/act (V3) - variables", () => {
    ...
    beforeEach(async () => {
        const navResponse = await navigateSession(varsSessionId, LOGIN_URL, getHeaders("3.0.0"));
    ...
    it("should substitute simple variable into typed value", async () => {
        const url = getBaseUrl();
        const openaiApiKey = requireEnv("OPENAI_API_KEY", OPENAI_API_KEY);
        const ctx = await fetchWithContext(`${url}/v1/sessions/${varsSessionId}/act`, {
            method: "POST",
            headers: {
                ...getHeaders("3.0.0"),
                "x-model-api-key": "",
        ...
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data?.result?.success, `act result should be success, got: ${ctx.body.data?.result?.message ?? "<no message>"}`, ctx);
        const emailValue = await readInputValue(EMAIL_XPATH);
        assert.equal(emailValue, "john@example.com", "Email field should contain substituted variable value");
    });
    it("should substitute rich variable (with description) into typed value", async () => {
        ...
        assert.equal(passwordValue, "secret123", "Password field should contain substituted variable value");
    });
});
// =============================================================================
...
describe("POST /v1/sessions/:id/act with SSE streaming (V3)", () => {
    it("should stream valid SSE events with correct structure", async () => {
        ...
        const { events } = ctx;
        assertWithContext(events.length >= 2, "Should have at least starting and finished events", ctx);
        // Verify starting event
        const startingEvent = assertEventExists(events, "starting", ctx);
        assert.equal(startingEvent.type, "system", "Starting event should be system type");
        // Verify finished event with result
        const finishedEvent = assertEventExists(events, "finished", ctx);
        assert.equal(finishedEvent.type, "system", "Finished event should be system type");
        assertWithContext(!!finishedEvent.data.result, "Finished event must have result", ctx);
        assert.equal(typeof finishedEvent.data.result.success, "boolean", "Result.success must be a boolean");
    });
    it("should include explicit SSE event names that match streamed statuses", async () => {
        ...
        assert.ok(events.every((event) => typeof event.event === "string" && event.event), "Every streamed frame should include an SSE event name");
        const startingEvent = events.find((event) => event.event === "starting");
        assert.ok(startingEvent, "Should include a starting SSE event");
        assert.equal(startingEvent.parsed?.data
            ?.status, "starting", "Starting SSE event should match the payload status");
        const finishedEvent = events.find((event) => event.event === "finished");
        assert.ok(finishedEvent, "Should include a finished SSE event");
        assert.equal(finishedEvent.parsed?.data
            ?.status, "finished", "Finished SSE event should match the payload status");
    });
