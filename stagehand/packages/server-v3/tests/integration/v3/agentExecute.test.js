...
import { assertFetchOk, assertFetchStatus, assertWithContext, createSession, createSessionWithCdp, endSession, fetchWithContext, GEMINI_API_KEY, getBaseUrl, getHeaders, getMainFrameId, HTTP_BAD_REQUEST, HTTP_OK, HTTP_UNPROCESSABLE_ENTITY, navigateSession, OPENAI_API_KEY, readTypedSSEStreamWithContext, requireEnv, } from "../utils.js";
// =============================================================================
...
describe("POST /v1/sessions/:id/agentExecute (V3) - Basic Config", () => {
    ...
    beforeEach(async () => {
        ...
        const navResponse = await navigateSession(sessionId, "https://example.com", headers);
    ...
    it("should execute agent with basic config (empty agentConfig)", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/agentExecute`, {
            method: "POST",
            headers,
            body: JSON.stringify({
                agentConfig: {},
        ...
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    });
    it("should execute agent with string agentConfig.model", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with string model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with object model config (provider + modelName)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with object model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with systemPrompt and maxSteps", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with systemPrompt and maxSteps should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute (V3) - Google Gemini with API key", () => {
    ...
    it("should execute agent with Google model object containing modelName and apiKey", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with Google model object should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with Google model object, systemPrompt, and maxSteps", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with Google model, systemPrompt and maxSteps should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute (V3) - OpenAI with API key", () => {
    ...
    it("should execute agent with OpenAI model object containing modelName and apiKey", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with OpenAI model object should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute (V3) - CUA flag compatibility", () => {
    ...
    it("should execute agent with cua: true and CUA model (valid combination)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with cua: true and CUA model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with cua: false and non-CUA model (valid combination)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with cua: false and non-CUA model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with cua: false and CUA model (works in non-CUA mode)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with cua: false and Google CUA model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should fail with cua: true and non-CUA model (invalid combination)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_UNPROCESSABLE_ENTITY, "V3 agent execute with cua: true and non-CUA model should fail");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
    ...
    it("should prefer mode over cua when both are provided", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with mode: dom and cua: true should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute (V3) - executionModel serialization", () => {
    ...
    it("should execute agent with string executionModel", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with string executionModel should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with object executionModel (modelName and apiKey)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with object executionModel should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with both model and executionModel", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with both model and executionModel should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute - agentConfig.mode (V3)", () => {
    ...
    it("should execute agent with mode: dom", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with mode: dom should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with mode: hybrid", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with mode: hybrid should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
    ...
    it("should execute agent with mode: cua and CUA model", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "V3 agent execute with mode: cua and CUA model should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(ctx.body.data !== undefined, "Response should have data", ctx);
        assertFetchOk(ctx.body.data.result !== undefined, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/agentExecute (V3) - variables", () => {
    const LOGIN_URL = "https://browserbase.github.io/stagehand-eval-sites/sites/login/";
    ...
    beforeEach(async () => {
        ...
        assert.equal(navResponse.status, HTTP_OK, "Navigate to login should succeed");
    });
    after(async () => {
        if (sessionId) {
            await endSession(sessionId, headers);
    ...
    async function readInputValue(xpath) {
    ...
    it("should substitute simple variables into agent tool calls", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/agentExecute`, {
            ...
            body: JSON.stringify({
                ...
                executeOptions: {
                    instruction: "Type %username% into the email field and %password% into the password field. Do not submit the form.",
                    maxSteps: 5,
                    variables: {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Agent execute with simple variables should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        const emailValue = await readInputValue(EMAIL_XPATH);
        const passwordValue = await readInputValue(PASSWORD_XPATH);
        assert.equal(emailValue, "john@example.com", "Email field should contain substituted username variable value");
        assert.equal(passwordValue, "secret123", "Password field should contain substituted password variable value");
    });
    it("should substitute rich variables (with descriptions) into agent tool calls", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Agent execute with rich variables should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        const emailValue = await readInputValue(EMAIL_XPATH);
        const passwordValue = await readInputValue(PASSWORD_XPATH);
        assert.equal(emailValue, "alice@example.com", "Email field should contain substituted username variable value");
        assert.equal(passwordValue, "rich-pw-456", "Password field should contain substituted password variable value");
    });
});
// =============================================================================
...
describe("POST /v1/sessions/:id/agentExecute with SSE streaming (V3)", () => {
    ...
    it("should stream SSE events with valid structure, sequence, and UUIDs", async () => {
        const url = getBaseUrl();
        const response = await fetch(`${url}/v1/sessions/${sessionId}/agentExecute`, {
            method: "POST",
            headers,
            body: JSON.stringify({
                agentConfig: {},
        ...
        // Verify event count
        assertWithContext(ctx.events.length >= 2, "Should have at least starting and finished events", ctx);
        // Verify event sequence
        const startingIndex = ctx.events.findIndex((e) => e.data.status === "starting");
        const connectedIndex = ctx.events.findIndex((e) => e.data.status === "connected");
        const finishedIndex = ctx.events.findIndex((e) => e.data.status === "finished");
        assertWithContext(startingIndex !== -1, "Should have a starting event", ctx);
        assertWithContext(connectedIndex !== -1, "Should have a connected event", ctx);
        assertWithContext(finishedIndex !== -1, "Should have a finished event", ctx);
        assertWithContext(startingIndex < connectedIndex, "Starting event must come before connected event", ctx);
        assertWithContext(connectedIndex < finishedIndex, "Connected event must come before finished event", ctx);
        // Verify event types
        const startingEvent = ctx.events[startingIndex];
        const finishedEvent = ctx.events[finishedIndex];
        assertWithContext(startingEvent !== undefined, "Starting event should exist", ctx);
        assertWithContext(finishedEvent !== undefined, "Finished event should exist", ctx);
        assertWithContext(startingEvent.type === "system", "Starting event should be system type", ctx);
        assertWithContext(finishedEvent.type === "system", "Finished event should be system type", ctx);
        assertWithContext(finishedEvent.data.result !== undefined, "Finished event must have result", ctx);
        // Verify UUID format
        const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
        for (const event of ctx.events) {
            assertWithContext(uuidRegex.test(event.id), `Event id should be a valid UUID format, got: ${event.id}`, ctx);
        }
    });
});
// =============================================================================
...
describe("POST /v1/sessions/:id/agentExecute - validation errors (V3)", () => {
    ...
    it("should return 400 when agentConfig is missing", async () => {
        ...
        assertFetchOk(!ctx.body?.success || ctx.body.error !== undefined, "Response should indicate failure", ctx);
    });
...
describe("POST /v1/sessions/:id/agentExecute (V3) - useSearch & toolTimeout", () => {
    ...
    it("should accept executeOptions.useSearch as boolean", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Agent execute with useSearch should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
    });
    it("should accept executeOptions.toolTimeout as number", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Agent execute with toolTimeout should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
    });
    it("should accept both useSearch and toolTimeout together", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Agent execute with both options should succeed");
        assertFetchOk(ctx.body !== null, "Response body should be parseable", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
    });
});
