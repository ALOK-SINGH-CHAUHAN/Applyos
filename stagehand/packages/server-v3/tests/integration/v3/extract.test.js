...
describe("POST /v1/sessions/:id/extract (V3)", () => {
    it("should extract data with instruction and schema", async () => {
        ...
        const ctx = await fetchWithContext(`${url}/v1/sessions/${sessionId}/extract`, {
            method: "POST",
        ...
        assert.equal(typeof ctx.body.data.result, "object", "Result should be an object");
        assertFetchOk("title" in ctx.body.data.result, "Result should have title property", ctx);
    });
    ...
    it("should extract with XPath selector in options", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Extract with XPath selector should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
    ...
    it("should extract without instruction (extract all)", async () => {
        ...
        assertFetchStatus(ctx, HTTP_OK, "Extract without instruction should succeed");
        assertFetchOk(ctx.body !== null, "Response should have body", ctx);
        assertFetchOk(ctx.body.success, "Response should indicate success", ctx);
        assertFetchOk(!!ctx.body.data, "Response should have data", ctx);
        assertFetchOk(!!ctx.body.data.result, "Response should have result", ctx);
...
describe("POST /v1/sessions/:id/extract with SSE streaming (V3)", () => {
    it("should stream valid SSE events with correct structure", async () => {
        ...
        assert.equal(typeof finishedEvent.data.result, "object", "Result must be an object");
        assertWithContext("title" in finishedEvent.data.result, "Result should have title property", ctx);
    });
    ...
    it("should extract data matching the provided schema", async () => {
        ...
        assert.equal(typeof finishedEvent.data.result.title, "string", "Extracted title should be a string");
    });
});
