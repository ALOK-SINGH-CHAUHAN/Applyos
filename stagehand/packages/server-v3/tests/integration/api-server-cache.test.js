...
import { assertFetchOk, assertFetchStatus, createSession, endSession, fetchWithContext, getBaseUrl, getHeaders, HTTP_OK, navigateSession, } from "./utils.js";
// Shared read-only session — extract is safe to re-use across tests.
let sessionId;
before(async () => {
    sessionId = await createSession(getHeaders("3.0.0"));
    const nav = await navigateSession(sessionId, "https://example.com", getHeaders("3.0.0"));
...
describe("browserbase-cache-bypass request header", () => {
    it("request with bypass header does not return cache HIT", async () => {
        ...
        assert.notEqual(cacheStatus, "HIT", "A bypassed request must not return a cache HIT");
    });
});
// ---------------------------------------------------------------------------
...
describe("browserbase-cache-status response header", () => {
    it("returns HIT or MISS when the header is present", async () => {
        ...
        const cacheStatus = ctx.headers.get("browserbase-cache-status");
        if (cacheStatus !== null) {
            assert.ok(cacheStatus === "HIT" || cacheStatus === "MISS", `browserbase-cache-status must be HIT or MISS, got: ${cacheStatus}`);
        }
    });
    it("returns HIT on a repeated identical request when caching is active", async () => {
        ...
        if (cacheStatus !== null) {
            assert.equal(cacheStatus, "HIT", "Repeated identical request should be a cache HIT");
        }
    });
});
