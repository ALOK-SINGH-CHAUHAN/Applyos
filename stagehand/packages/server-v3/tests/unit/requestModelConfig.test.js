...
import { getRequestModelConfig, getStagehandInitModelConfig, } from "../../src/lib/header.js";
...
describe("withModelApiKeyFallback", () => {
    ...
    it("still uses request header API keys as a fallback for simple model configs", () => {
        assert.deepEqual(withModelApiKeyFallback({ modelName: "openai/gpt-4.1-mini" }, "sk-header"), {
            modelName: "openai/gpt-4.1-mini",
            apiKey: "sk-header",
        });
    });
    it("does not overwrite per-model API keys with request header API keys", () => {
        assert.deepEqual(withModelApiKeyFallback({
            modelName: "openai/gpt-4.1-mini",
            apiKey: "sk-body",
        }, "sk-header"), {
            modelName: "openai/gpt-4.1-mini",
