...
import { formatId, formatUtcDateTime, outputFormatFlags, outputTable, resolveOutputFormat, } from "../../../lib/output.js";
export default class SessionsList extends BrowseCommand {
    static description = "List Browserbase sessions.";
    static examples = [
...
function outputSessionsTable(sessions, options) {
    const visibleSessions = sessions.slice(0, options.limit);
    if (visibleSessions.length === 0) {
        console.log("No sessions found.");
        return;
    }
    outputTable(visibleSessions, [
        {
            header: "ID",
            maxWidth: 12,
    ...
    if (visibleSessions.length < sessions.length) {
        console.log(`Showing ${visibleSessions.length} of ${sessions.length} sessions returned. Use --limit, --all, --status, --q, or --json.`);
    }
}
...
function formatKeepAlive(value) {
    if (value === undefined) {
        return "-";
    }
    return value ? "yes" : "no";
