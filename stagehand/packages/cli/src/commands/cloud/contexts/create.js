...
export default class ContextsCreate extends BrowseCommand {
    static description = "Create a Browserbase context. Pass --name to save a local alias you can reuse instead of the context ID.";
    static examples = [
    ...
    async run() {
        ...
        if (name !== undefined) {
            ...
            if (await getContextAlias(name)) {
                fail(`A context named "${name}" already exists locally. Choose another name or remove it with "browse cloud contexts delete ${name}".`);
            }
        }
