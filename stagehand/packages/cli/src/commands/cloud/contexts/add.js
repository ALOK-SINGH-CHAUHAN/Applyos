...
import { contextNameRequirement, getContextAlias, isValidContextName, saveContextAlias, } from "../../../lib/cloud/contexts-store.js";
import { fail } from "../../../lib/errors.js";
import { outputJson } from "../../../lib/output.js";
import { BrowseCommand } from "../../../base.js";
export default class ContextsAdd extends BrowseCommand {
    static description = "Save a local name for an existing Browserbase context ID so you can reuse it by name (e.g. an ID a teammate shared or one created on another device).";
    static examples = [
    ...
    async run() {
        ...
        if (!flags.force && (await getContextAlias(args.name))) {
            fail(`A context named "${args.name}" already exists locally. Pass --force to overwrite, ` +
