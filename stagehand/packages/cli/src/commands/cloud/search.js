...
export default class Search extends BrowseCommand {
    static description = "Search the web using the Browserbase Search API.";
    static examples = [
    ...
    async run() {
        ...
        const result = await requestBrowserbaseJson(toApiOptions(flags), "/v1/search", {
            method: "POST",
        ...
        if (flags.output) {
            ...
            if (outputFormat === "table") {
                console.log(`Wrote ${result.results.length} results for "${result.query}" to ${flags.output}.`);
                return;
            }
            outputJson({
                ok: true,
...
function outputSearchTable(results, options) {
    if (results.length === 0) {
        console.log("No search results found.");
        return;
    }
    outputTable(results.map((result, index) => ({ ...result, index: index + 1 })), [
        {
            align: "right",
            header: "#",
            maxWidth: 3,
