...
export default class SkillsAdd extends BrowseCommand {
    ...
    async run() {
        ...
        if (!args.skill) {
            fail("Missing skill id. Pass a skill in the form <domain>/<task>, e.g. `browse skills add yelp.com/extract-reviews`. Run `browse skills find <query>` to discover skills.", 2, { resultCode: "invalid_skill_id" });
        }
