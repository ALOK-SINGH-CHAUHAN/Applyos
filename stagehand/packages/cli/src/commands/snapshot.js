...
export default class Snapshot extends BrowseCommand {
    static description = "Print the active page accessibility snapshot and cache refs for element commands. Pass --full to also include the ref maps (xpathMap, urlMap).";
    static examples = [
        "browse snapshot",
    ...
    async run() {
        ...
        if (flags.compact && process.stderr.isTTY) {
            this.warn("`--compact` is deprecated and has no effect; use `--full` to include the ref maps.");
        }
        await runDriverCommandFromFlags("snapshot", {
            full: flags.full,
            filter: flags.filter,
