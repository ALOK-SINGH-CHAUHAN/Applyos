...
export async function maybeAutoUpdateCli(currentVersion, env = process.env, options = {}) {
    if (env.BROWSE_DISABLE_UPDATE_CHECK === "1" ||
...
export async function refreshUpdateCheckCache(env = process.env, options = {}) {
    const cachePath = resolveUpdateCheckPath(env, options);
    if (!cachePath) {
        return;
    }
...
async function readFreshUpdateCheckCache(cachePath) {
...
async function readUpdateCheckCache(cachePath) {
    try {
...
async function writeUpdateCheckCache(cachePath, cache) {
    try {
...
function resolveUpdateCheckPath(env, options = {}) {
    const configured = options.cacheFile ??
        env.BROWSE_UPDATE_CHECK_FILE ??
        env.BB_UPDATE_CHECK_FILE;
    return configured && configured.length > 0 ? configured : null;
}
function spawnBackgroundUpdateCheck(env, cachePath) {
    try {
...
function resolveUpdateCheckWorkerPath() {
    const extension = import.meta.url.endsWith(".ts") ? "ts" : "js";
    return fileURLToPath(new URL(`../update-check-worker.${extension}`, import.meta.url));
}
function writeUpdateNotice(currentVersion, latestVersion) {
...
async function fetchLatestCliVersion() {
    ...
    try {
        const latestUrl = new URL(`${encodeURIComponent(CLI_PACKAGE_NAME)}/latest`, DEFAULT_NPM_REGISTRY_BASE_URL);
