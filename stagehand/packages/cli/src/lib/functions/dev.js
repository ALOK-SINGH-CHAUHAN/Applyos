...
import { functionsRequest, resolveEntrypoint, resolveFunctionsApiConfig, } from "./shared.js";
...
class InvocationBridge {
    ...
    holdNextConnection(response, corsHeaders) {
    ...
    async completeWithError(requestId, payload) {
        ...
        sendJson(this.invokeConnection.response, 500, { error: payload }, this.invokeConnection.corsHeaders);
        try {
            await this.cleanupSession();
        }
        catch (error) {
    ...
    triggerInvocation(functionName, params, context, corsHeaders, response) {
...
class RuntimeProcess {
    ...
    async start() {
        ...
        const child = spawn(nodeExecutable, [tsxCli, "watch", "--clear-screen=false", this.entrypoint], {
            cwd: process.cwd(),
            env: {
                ...process.env,
...
export async function startFunctionsDevServer(options) {
    ...
    await runtime.start();
    const runtimeConnected = await waitForRuntime(bridge, manifestStore, getRuntimeStartupTimeoutMs());
    const output = {
        ok: runtimeConnected,
        runtimeConnected,
...
async function waitForRuntime(bridge, manifestStore, timeoutMs) {
...
function getRuntimeStartupTimeoutMs() {
...
async function routeRequest(request, response, bridge, manifestStore, sessionManager) {
    ...
    if (method === "POST" && invokeMatch?.[1]) {
        ...
        const accepted = bridge.triggerInvocation(functionName, params, { session }, corsHeaders, response);
        if (!accepted) {
    ...
    if (method === "POST" && responseMatch?.[1]) {
        ...
        try {
            ...
            sendJson(response, 400, completed ? { error: message } : { error: "Request ID mismatch." }, corsHeaders);
            return;
        }
        const completed = await bridge.completeWithSuccess(requestId, payload);
        sendJson(response, completed ? 202 : 400, completed ? { ok: true } : { error: "Request ID mismatch." }, corsHeaders);
        return;
    }
...
function sendJson(response, statusCode, body, corsHeaders) {
...
function sendNoContent(response, statusCode, corsHeaders) {
