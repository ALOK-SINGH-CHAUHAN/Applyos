...
export async function installSkill(rawSkillId) {
    ...
    if (files.status === "not_found") {
        fail(`Skill "${skill.id}" not found in the catalog. Run \`browse skills find ${skill.domain}\` to discover available skills, or \`browse skills list\` to browse them.`, 1, { resultCode: "skill_not_found" });
    }
...
async function runSkillsInstall(npxPath, args) {
    ...
    if (result.timedOut) {
        fail(`Skill install timed out after ${Math.round(timeoutMs / 1000)}s waiting for \`npx skills add\`. Check your network connection and rerun \`browse skills add\`.`, 1, { resultCode: "skill_install_timeout" });
    }
...
export function bundledCliSkillPath() {
...
export function quoteForCmdShell(token) {
    if (token === "") {
        return '""';
    }
...
// Exported for tests.
export async function spawnPassthrough(command, args, timeoutMs = installTimeoutMs()) {
