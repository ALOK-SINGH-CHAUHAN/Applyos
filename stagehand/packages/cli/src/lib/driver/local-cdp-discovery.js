...
export function getChromeUserDataDirs() {
...
export function buildDevToolsWsUrl(port, wsPath) {
...
async function probeJsonVersion(port) {
...
async function verifyCdpWebSocket(wsUrl) {
    return new Promise((resolve) => {
...
async function resolveDevToolsActivePortUrl(port, userDataDirs) {
...
export async function resolveWsTargetFromPort(port, options = {}) {
    ...
    if (await verifyCdpWebSocket(fallback))
        return fallback;
    throw new Error(`Unable to resolve CDP endpoint from port ${port}. Is Chrome running with remote debugging?`);
}
export async function discoverLocalCdp(options = {}) {
    const candidates = [];
    const userDataDirs = options.userDataDirs ?? getChromeUserDataDirs();
    for (const dir of userDataDirs) {
        const info = await readDevToolsActivePort(dir);
        if (!info || !(await isPortReachable(info.port)))
            continue;
        candidates.push({
