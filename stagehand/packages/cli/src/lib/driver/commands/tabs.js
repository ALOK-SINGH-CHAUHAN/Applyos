...
export const tabHandlers = {
    ...
    async "tab.new"(manager, params) {
        ...
        return {
            ...
            index: context
                .pages()
                .findIndex((candidate) => candidate.targetId() === page.targetId()),
...
async function resolveTab(manager, tab) {
    const context = await manager.browserContext();
    const pages = context.pages();
    ...
    const targetIndex = pages.findIndex((page) => page.targetId() === tab);
    if (targetIndex === -1) {
...
function resolveActiveTab(pages, active) {
    const activeTargetId = active?.targetId();
