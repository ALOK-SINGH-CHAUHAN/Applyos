...
export function looksLikeContextId(value) {
...
export function contextNameRequirement() {
...
export function contextsStorePath(env = process.env) {
...
export async function listContextAliases(env = process.env) {
    const store = await readStore(env);
    return Object.entries(store.contexts)
        .map(([name, alias]) => ({ name, ...alias }))
        .sort((a, b) => a.name.localeCompare(b.name));
}
export async function getContextAlias(name, env = process.env) {
    const store = await readStore(env);
    return store.contexts[name];
}
export async function saveContextAlias(name, alias, env = process.env) {
    const store = await readStore(env);
    store.contexts[name] = alias;
    await writeStore(store, env);
}
export async function removeContextAlias(name, env = process.env) {
    const store = await readStore(env);
 ...
 */
export async function removeContextAliasesById(id, env = process.env) {
    const store = await readStore(env);
 ...
 */
export async function resolveContextRef(ref, env = process.env) {
 ...
 * closest saved names.
 */
export async function resolveContextRefDetailed(ref, env = process.env) {
    const store = await readStore(env);
 ...
 * loosely.
 */
export function closeContextNameMatches(ref, names, limit = 3) {
    return names
