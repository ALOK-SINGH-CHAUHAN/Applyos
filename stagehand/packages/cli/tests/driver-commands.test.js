...
describe("driver commands", () => {
    it("registers the native driver command handlers without legacy underscore aliases", () => {
        expect([...DRIVER_COMMAND_NAMES].sort()).toEqual(expect.arrayContaining([
            "click",
            "mouse.click",
            "snapshot",
            "tab.switch",
        ...
        expect([...DRIVER_COMMAND_NAMES]).not.toEqual(expect.arrayContaining([
            "click_xy",
            "tab_switch",
            "tab_close",
            "network_enable",
    ...
    it("treats headed and headless as explicit local target choices", () => {
        ...
        expect(hasExplicitDriverTarget({
    ...
    it("reuses an existing daemon when a broad mode flag matches", async () => {
        ...
        try {
            ...
            await expect(resolveTargetForCommand("reuse-local", { headless: true, local: true })).resolves.toEqual({
                headless: true,
                kind: "managed-local",
            });
            await expect(resolveTargetForCommand("reuse-local", { remote: true })).resolves.toEqual({ kind: "remote" });
        }
        finally {
    ...
    it("routes CDP targets through the daemon so session state persists", async () => {
        ...
        try {
            ...
            await expect(runDriverCommandWithTarget("cdp-state", target, "snapshot", {
                compact: true,
            })).resolves.toEqual({
                ok: true,
            });
            ...
            expect(runDriverCommandViaDaemon).toHaveBeenCalledWith("cdp-state", "snapshot", { compact: true });
    ...
    it("rejects unknown daemon command names at the protocol boundary", () => {
        expect(() => parseRequest(JSON.stringify({ command: "snapshot", id: "1", type: "command" }))).not.toThrow();
        expect(() => parseRequest(JSON.stringify({ command: "not.real", id: "1", type: "command" }))).toThrow();
    });
    ...
    it("rejects invalid wait timeout values before calling the page", async () => {
        ...
        await expect(runtimeHandlers.wait(manager, { arg: "100abc", type: "timeout" })).rejects.toThrow("wait timeout requires a non-negative integer");
        await expect(runtimeHandlers.wait(manager, { arg: "-1", type: "timeout" })).rejects.toThrow("wait timeout requires a non-negative integer");
        expect(page.waitForTimeout).not.toHaveBeenCalled();
        await expect(runtimeHandlers.wait(manager, { arg: "100", type: "timeout" })).resolves.toEqual({ waited: true });
    ...
    it("accepts fractional viewport scale values", async () => {
        ...
        try {
            ...
            const result = await runCli(["viewport", "1024", "768", "--scale", "1.5", "--session", session], {
    ...
    it("filters and trims snapshot output without changing ref syntax", () => {
        ...
        expect(formatSnapshotTree(tree, { filter: "submit" })).toBe(["- page:", "  - main:", "    - button [0-2]: Submit order"].join("\n"));
    ...
    it("keeps network responses when loading finishes before request file writes", async () => {
        ...
        try {
            ...
            const responsePath = join(daemonDir, "race-network", "000-GET-example.com-fast", "response.json");
...
function createFakeTabManager(targetIds, activeIndex) {
    let pages = [];
    let active = null;
    ...
    return {
        ...
        manager: {
            ...
            safeTitle: async (page) => page.title(),
        },
        pages,
    };
}
