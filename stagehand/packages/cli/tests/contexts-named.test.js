...
describe("named contexts (end to end through the CLI)", () => {
    it("creates by name, resolves the name everywhere, and prunes on delete", async () => {
        ...
        try {
            ...
            const session = await runCli(["cloud", "sessions", "create", "--context-id", "github", "--persist"], { env });
            expect(session.exitCode).toBe(0);
            const sessionRequest = server.requests.find((r) => r.method === "POST" && pathOf(r) === "/v1/sessions");
            ...
            expect(server.requests.some((r) => r.method === "GET" && pathOf(r) === `/v1/contexts/${CONTEXT_ID}`)).toBe(true);
    ...
    it("names an existing context with `add`, resolves it, and --force repoints", async () => {
        ...
        try {
            ...
            expect(got.exitCode).toBe(0);
            expect(server.requests.some((r) => r.method === "GET" && pathOf(r) === `/v1/contexts/${existingId}`)).toBe(true);
            ...
            const forced = await runCli(["cloud", "contexts", "add", "team-login", `  ${newId}  `, "--force"], { env });
            expect(forced.exitCode).toBe(0);
            expect(JSON.parse(forced.stdout)).toEqual({
                name: "team-login",
                id: newId,
            });
            expect(JSON.parse(await readFile(storePath, "utf8")).contexts).toMatchObject({ "team-login": { id: newId } });
        }
        finally {
            await server.close();
        }
    ...
    it("passes an unrecognized raw id through to the API (raw-id compatibility)", async () => {
        ...
        try {
            ...
            expect(got.exitCode).toBe(0);
            expect(server.requests.some((r) => r.method === "GET" && pathOf(r) === `/v1/contexts/${rawId}`)).toBe(true);
        }
        finally {
            await server.close();
        }
