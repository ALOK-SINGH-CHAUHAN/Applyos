...
import { jsonResponse, startFakeBrowserbaseServer, } from "./helpers/fake-browserbase-server.js";
...
describe("functions API contracts", () => {
    ...
    it("prints publish dry-run archive metadata", async () => {
        ...
        const fakeBin = await createFakePackageManagerBin("npm", "#!/bin/sh\necho npm should not run during dry-run >&2\nexit 99\n");
...
describe("functions scaffolding and local dev", () => {
    itPosix("scaffolds a Functions project", async () => {
        ...
        const result = await runCli(["functions", "init", "demo-function", "--package-manager", "pnpm"], {
            cwd,
            env: {
                PATH: `${fakeBin}:${process.env.PATH}`,
            },
        ...
        expect(await readFile(join(cwd, "demo-function", ".env"), "utf8")).toContain("BROWSERBASE_API_KEY=");
    });
    it("runs a local dev server and invokes a function", async () => {
        ...
        await withServer(async (request, response) => {
            ...
            const optionsResponse = await fetch(`http://127.0.0.1:${port}/v1/functions/test-function/invoke`, {
                headers: {
            ...
            const invokeResponse = await fetch(`http://127.0.0.1:${port}/v1/functions/test-function/invoke`, {
                method: "POST",
                headers: {
...
async function createFunctionFixture(prefix) {
...
async function createFakePackageManagerBin(name = "pnpm", contents = "#!/bin/sh\nexit 0\n") {
...
async function writeRuntimeEntrypoint(cwd, options = {}) {
    await writeFile(join(cwd, "runtime-entry.mjs"), `
...
async function writeExitingRuntimeEntrypoint(cwd) {
    await writeFile(join(cwd, "runtime-entry.mjs"), `
...
async function writeIdleRuntimeEntrypoint(cwd) {
    await writeFile(join(cwd, "runtime-entry.mjs"), `
setInterval(() => {}, 1_000);
`);
}
...
async function invokeLocalFunction(port, params) {
...
function expectRequest(request, method, path, apiKey) {
    expect(request).toBeDefined();
    expect(request?.method).toBe(method);
    expect(request?.path).toBe(path);
    expect(request?.headers["x-bb-api-key"]).toBe(apiKey);
}
async function waitForStdout(child, text, timeoutMs = 10_000) {
    return await new Promise((resolvePromise, reject) => {
        let stdout = "";
        let stderr = "";
        const timer = setTimeout(() => {
            reject(new Error(`Timed out waiting for stdout to include ${text}. Saw stdout: ${stdout}. Saw stderr: ${stderr}`));
        }, timeoutMs);
        ...
        child.once("exit", (code) => {
            clearTimeout(timer);
            reject(new Error(`Dev server exited early with code ${code}. Saw stdout: ${stdout}. Saw stderr: ${stderr}`));
        });
