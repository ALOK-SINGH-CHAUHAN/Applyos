...
describe("skills", () => {
    ...
    it("prints exact skill matches as concise detail output", async () => {
        ...
        cleanupServers.push(server);
        const result = await runCli(["skills", "find", "yelp.com/extract-reviews", "--format", "table"], {
            env: {
                BROWSE_SKILLS_API_BASE_URL: baseUrl,
            },
        });
    ...
    it("limits broad skill find table matches", async () => {
        ...
        cleanupServers.push(server);
        const result = await runCli(["skills", "find", "restaurant", "--format", "table", "--limit", "1"], {
            env: {
                BROWSE_SKILLS_API_BASE_URL: baseUrl,
            },
        });
    ...
    it("reports catalog API failures", async () => {
        ...
        expect(result.exitCode).toBe(1);
        expect(result.stderr).toContain("Failed to fetch skills: 500 Internal Server Error: catalog unavailable");
    });
    itPosix("fails cleanly when a non-generated skill is missing from the catalog", async () => {
        ...
        expect(result.exitCode).toBe(1);
        expect(result.stderr).toContain('Skill "amazon.com/buy-something-fake" not found in the catalog');
        expect(result.stderr).toContain("browse skills find amazon.com");
        // It must NOT have shelled out to clone the browse.sh repo.
        await expect(readFile(logPath, "utf8").catch(() => "")).resolves.not.toContain("browserbase/browse.sh");
    });
    itPosix("installs suffix-shaped catalog skills from GitHub when the file API returns 404", async () => {
        ...
        await expect(readFile(logPath, "utf8")).resolves.toContain("--yes skills add browserbase/browse.sh --skill airline.example/book-flight");
    });
    itPosix("installs suffix-shaped catalog skills from GitHub when the file API is unavailable and no Blob fallback exists", async () => {
    ...
    itPosix("downloads generated skills from the Browse.sh file API before installing", async () => {
        ...
        await writeNpxStub(stubDir, logPath);
        const { server, baseUrl } = await startFakeSkillServer({
            "/api/skills/mcdonalds.order.online/order-delivery-42q71n/files": (origin) => JSON.stringify({
        ...
        const installPath = join(configHome, "browserbase", "skills", "mcdonalds.order.online", "order-delivery-42q71n");
        await expect(readFile(join(installPath, "SKILL.md"), "utf8")).resolves.toContain("name: order-delivery");
    ...
    itPosix("falls back to direct SKILL.md download when the file API is unavailable for a suffix-shaped skill", async () => {
...
const catalogSkills = [
    {
        ...
        source: "browserbase",
        sourceUrl: "https://github.com/browserbase/browse.sh/blob/main/skills/yelp.com/extract-reviews/SKILL.md",
        tags: ["yelp", "reviews"],
    ...
    },
    {
        ...
        source: "browserbase",
        sourceUrl: "https://github.com/browserbase/browse.sh/blob/main/skills/opentable.com/check-availability/SKILL.md",
...
async function writeNpxStub(stubDir, logPath) {
    const stubPath = join(stubDir, "npx");
    await writeFile(stubPath, ["#!/bin/sh", 'printf \'%s\\n\' "$*" >> "$BB_STUB_LOG"', "exit 0", ""].join("\n"));
    await chmod(stubPath, 0o755);
    process.env.BB_STUB_LOG = logPath;
}
async function startFakeSkillServer(files) {
    const server = createServer((request, response) => {
        ...
        const body = typeof bodySource === "function"
            ? bodySource(origin, requestUrl)
            : typeof bodySource === "object"
                ? (bodySource.body ?? "")
                : bodySource;
        const contentType = typeof bodySource === "object"
