...
import { jsonResponse, startFakeBrowserbaseServer, textResponse, } from "./helpers/fake-browserbase-server.js";
...
describe("CLI telemetry", () => {
    it("emits invoked and completed events for successful commands", async () => {
        ...
        try {
            ...
            expect(payloads.every((payload) => payload.properties.$process_person_profile === false)).toBe(true);
    ...
    it("emits completion telemetry for oclif parse failures without raw args", async () => {
        ...
        try {
            ...
            const result = await runCli(["cloud", "search", "sensitive search phrase", "--num-results", "999"], { env: telemetryEnv(telemetryServer, installIdFile) });
    ...
    it("never attaches unvalidated skill arguments to telemetry", async () => {
        ...
        try {
            ...
            const result = await runCli(["skills", "add", "../secret-local-path/oops"], { env: telemetryEnv(telemetryServer, installIdFile) });
            expect(result.exitCode).toBe(1);
            const payloads = telemetryPayloads(telemetryServer);
            const completedPayload = findPayload(payloads, "cli.command_completed");
            expect(completedPayload.properties.skill_id).toBe(null);
    ...
    it("reuses a stable anonymous install id across runs", async () => {
        ...
        try {
            ...
            const distinctIds = new Set(telemetryPayloads(telemetryServer)
...
function telemetryEnv(telemetryServer, installIdFile) {
    return {
        AI_AGENT: "",
        ANTIGRAVITY_AGENT: "",
        AUGMENT_AGENT: "",
...
function findPayload(payloads, event) {
...
async function tempInstallIdFile(prefix) {
