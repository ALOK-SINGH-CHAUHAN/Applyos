...
import { closeContextNameMatches, contextsStorePath, getContextAlias, isValidContextName, listContextAliases, looksLikeContextId, removeContextAlias, removeContextAliasesById, resolveContextRef, resolveContextRefDetailed, saveContextAlias, } from "../src/lib/cloud/contexts-store.js";
let configDir;
let env;
beforeEach(async () => {
...
describe("contexts store", () => {
    ...
    it("drops malformed entries on read instead of trusting them", async () => {
        await writeFile(contextsStorePath(env), JSON.stringify({
            version: 1,
            contexts: {
        ...
        expect(await resolveContextRef("45ed525f-63a5-490d-b4c4-853f50643b90", env)).toBe("45ed525f-63a5-490d-b4c4-853f50643b90");
    });
