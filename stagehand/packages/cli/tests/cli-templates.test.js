...
describe("templates commands", () => {
    ...
    it("prints exact template details from a slug", async () => {
        await withTemplatesApi(async ({ baseUrl, requests }) => {
            const result = await runCli(["templates", "find", "google-trends-keywords", "--format", "table"], {
                env: { BROWSERBASE_TEMPLATES_API: baseUrl },
            });
            expect(result.exitCode).toBe(0);
            expect(requests).toHaveLength(1);
    ...
    itPosix("clones TypeScript templates with create-browser-app via npx", async () => {
        ...
        await writeExecutable(join(stubDir, "npx"), [
            "#!/bin/sh",
            'if [ "$1" = "--version" ]; then',
            "  exit 0",
            "fi",
        ...
        await withTemplatesApi(async ({ baseUrl, requests }) => {
            ...
            expect(await readFile(logPath, "utf8")).toContain("npx create-browser-app@latest my-scraper --template amazon-product-scraping");
            expect(result.stdout).toContain(`Scaffolding typescript/amazon-product-scraping into ${dest}...`);
    ...
    itPosix("clones Python templates with create-browser-app via uvx", async () => {
        ...
        const logPath = join(stubDir, "commands.log");
        await writeExecutable(join(stubDir, "uvx"), [
            "#!/bin/sh",
            'if [ "$1" = "--version" ]; then',
            "  exit 0",
            "fi",
        ...
        await withTemplatesApi(async ({ baseUrl }) => {
            ...
            expect(result.exitCode).toBe(0);
            expect(await readFile(logPath, "utf8")).toContain("uvx create-browser-app py-scraper --template amazon-product-scraping");
    ...
    itPosix("prints clone results as JSON", async () => {
        ...
        await withTemplatesApi(async ({ baseUrl }) => {
            ...
            const result = await runCli(["templates", "clone", "amazon-product-scraping", dest, "--json"], {
                env: {
                    BROWSERBASE_TEMPLATES_API: baseUrl,
                    PATH: `${stubDir}:${process.env.PATH ?? ""}`,
                },
...
async function withTemplatesApi(callback) {
    const server = await startFakeBrowserbaseServer((request, response) => {
        ...
        const filteredTemplates = templates.filter((template) => {
            return ((!category || template.category.includes(category)) &&
