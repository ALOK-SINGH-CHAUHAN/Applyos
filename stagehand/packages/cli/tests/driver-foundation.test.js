...
import { ensureRuntimeDir, getLockPath, getPidPath, getSocketPath, runtimeDir, sanitizeSessionName, } from "../src/lib/driver/daemon/paths.js";
import { ensureDriverDaemon, getDriverStatus, openViaDaemon, } from "../src/lib/driver/daemon/client.js";
import { collectForwardedEnv, forwardedEnvSignature, } from "../src/lib/driver/daemon/forwarded-env.js";
...
describe("driver foundation", () => {
    ...
    it("resolves explicit driver modes", async () => {
        ...
        await expect(resolveConnectionTarget({ "auto-connect": true })).resolves.toEqual({ kind: "auto-connect" });
        await expect(resolveConnectionTarget({
    ...
    it("rejects ambiguous mode flags", async () => {
        await expect(resolveConnectionTarget({ headless: true, headed: true, local: true })).rejects.toThrow("Pass either --headed or --headless");
        await expect(resolveConnectionTarget({ local: true, remote: true })).rejects.toThrow("Pass either --local or --remote");
        await expect(resolveConnectionTarget({ "auto-connect": true, remote: true })).rejects.toThrow("--auto-connect cannot be combined with --remote");
        await expect(resolveConnectionTarget({
            "auto-connect": true,
            "chrome-arg": ["--no-focus-on-navigate"],
        })).rejects.toThrow("--auto-connect cannot be combined with --chrome-arg");
        await expect(resolveConnectionTarget({ "auto-connect": true, local: true })).rejects.toThrow("--auto-connect cannot be combined with --local");
        await expect(resolveConnectionTarget({ "auto-connect": true, headed: true })).rejects.toThrow("--auto-connect cannot be combined with --headed");
        await expect(resolveConnectionTarget({ "auto-connect": true, headless: true })).rejects.toThrow("--auto-connect cannot be combined with --headless");
        await expect(resolveConnectionTarget({
            "chrome-arg": ["--no-focus-on-navigate"],
            remote: true,
        })).rejects.toThrow("--remote cannot be combined with --chrome-arg");
        await expect(resolveConnectionTarget({ remote: true, headed: true })).rejects.toThrow("--remote cannot be combined with --headed");
        await expect(resolveConnectionTarget({ remote: true, headless: true })).rejects.toThrow("--remote cannot be combined with --headless");
        await expect(resolveConnectionTarget({
            cdp: "9222",
            "chrome-arg": ["--no-focus-on-navigate"],
        })).rejects.toThrow("--cdp cannot be combined with --chrome-arg");
        await expect(resolveConnectionTarget({ cdp: "9222", local: true })).rejects.toThrow("--cdp cannot be combined with --local");
        await expect(resolveConnectionTarget({ cdp: "9222", remote: true })).rejects.toThrow("--cdp cannot be combined with --remote");
        await expect(resolveConnectionTarget({ cdp: "9222", "auto-connect": true })).rejects.toThrow("--cdp cannot be combined with --auto-connect");
        await expect(resolveConnectionTarget({ cdp: "9222", headed: true })).rejects.toThrow("--cdp cannot be combined with --headed");
        await expect(resolveConnectionTarget({ cdp: "9222", headless: true })).rejects.toThrow("--cdp cannot be combined with --headless");
        await expect(resolveConnectionTarget({ "target-id": "target-1" })).rejects.toThrow("--target-id requires --cdp");
    });
    it("rejects combining --no-default-chrome-args with --ignore-default-chrome-arg", async () => {
        await expect(resolveConnectionTarget({
            ...
            "no-default-chrome-args": true,
            local: true,
        })).rejects.toThrow("--no-default-chrome-args cannot be combined with --ignore-default-chrome-arg.");
    });
    it("rejects local-only display flags for implicit remote mode", async () => {
        const previousApiKey = process.env.BROWSERBASE_API_KEY;
        process.env.BROWSERBASE_API_KEY = "test-key";
        try {
            await expect(resolveConnectionTarget({ headed: true })).rejects.toThrow("remote mode cannot be combined with --headed");
            await expect(resolveConnectionTarget({ headless: true })).rejects.toThrow("remote mode cannot be combined with --headless");
            await expect(resolveConnectionTarget({
                "chrome-arg": ["--no-focus-on-navigate"],
    ...
    it("resolves --remote with --verified and --proxies", async () => {
        ...
        await expect(resolveConnectionTarget({ remote: true, verified: true })).resolves.toEqual({ kind: "remote", verified: true });
        await expect(resolveConnectionTarget({ proxies: true, remote: true })).resolves.toEqual({ kind: "remote", proxies: true });
        await expect(resolveConnectionTarget({ proxies: true, remote: true, verified: true })).resolves.toEqual({ kind: "remote", proxies: true, verified: true });
    });
    it("requires --remote for --verified and --proxies", async () => {
        await expect(resolveConnectionTarget({ verified: true })).rejects.toThrow("--verified requires --remote");
        await expect(resolveConnectionTarget({ proxies: true })).rejects.toThrow("--proxies requires --remote");
        // plural subject keeps the plural verb
        await expect(resolveConnectionTarget({ proxies: true, verified: true })).rejects.toThrow("--verified and --proxies require --remote");
        // verified/proxies must be explicit about remote even when --cdp/--local is set
        await expect(resolveConnectionTarget({ cdp: "9222", verified: true })).rejects.toThrow("--verified requires --remote");
        await expect(resolveConnectionTarget({ local: true, proxies: true })).rejects.toThrow("--proxies requires --remote");
    });
    ...
    it("makes remote verified/proxies sticky for session reuse", () => {
        ...
        expect(targetsCompatible({ kind: "remote", verified: true, proxies: true }, { kind: "remote", verified: true, proxies: true })).toBe(true);
        ...
        expect(targetsCompatible({ kind: "remote", verified: true }, { kind: "remote", verified: true, proxies: true })).toBe(false);
    });
    it("only reuses daemon sessions for matching launch targets", () => {
        expect(targetsCompatible({
            chromeArgs: ["--no-focus-on-navigate"],
            headless: false,
            kind: "managed-local",
        }, {
        ...
        expect(targetsCompatible({ headless: false, kind: "managed-local" }, { chromeArgs: [], headless: false, kind: "managed-local" })).toBe(true);
        expect(targetsCompatible({
            chromeArgs: ["--no-focus-on-navigate"],
            headless: false,
            kind: "managed-local",
        ...
        expect(targetsCompatible({ headless: false, ignoreDefaultArgs: true, kind: "managed-local" }, { headless: false, ignoreDefaultArgs: true, kind: "managed-local" })).toBe(true);
        expect(targetsCompatible({
            headless: false,
            ignoreDefaultArgs: ["--enable-automation"],
            kind: "managed-local",
        ...
        expect(targetsCompatible({ headless: false, ignoreDefaultArgs: true, kind: "managed-local" }, {
            headless: false,
            ignoreDefaultArgs: ["--enable-automation"],
            kind: "managed-local",
        })).toBe(false);
        expect(targetsCompatible({ headless: false, kind: "managed-local" }, { headless: false, ignoreDefaultArgs: true, kind: "managed-local" })).toBe(false);
        expect(targetsCompatible({ endpoint: "ws://127.0.0.1:9222/devtools/browser/a", kind: "cdp" }, { endpoint: "ws://127.0.0.1:9222/devtools/browser/a", kind: "cdp" })).toBe(true);
        expect(targetsCompatible({ endpoint: "ws://127.0.0.1:9222/devtools/browser/a", kind: "cdp" }, { endpoint: "ws://127.0.0.1:9222/devtools/browser/b", kind: "cdp" })).toBe(false);
    });
    ...
    it.runIf(process.platform !== "win32")("creates the runtime directory with owner-only permissions", async () => {
    ...
    it("exposes descriptive help for top-level driver commands", async () => {
        for (const command of ["open", "status", "stop"]) {
            ...
            if (command === "open")
                expect(result.stdout).toContain("--chrome-arg");
        }
    });
    ...
    it("checks target compatibility after waiting for the daemon lock", async () => {
        ...
        try {
            ...
            await expect(ensureDriverDaemon({ session, target: { kind: "remote" } })).rejects.toThrow(`Session "${session}" is already running in managed-local mode.`);
            if (serverStartError)
    ...
    it("times out resolving HTTP CDP endpoints", async () => {
        ...
        try {
            ...
            await expect(resolveWsTarget(`http://127.0.0.1:${address.port}`, {
                httpTimeoutMs: 50,
    ...
    it("closes Stagehand when initialization fails", async () => {
        ...
        try {
            const { DriverSessionManager: MockedDriverSessionManager } = await import("../src/lib/driver/session-manager.js");
    ...
    it("creates a page for open when the initialized session has none", async () => {
        ...
        vi.spyOn(manager, "ensureInitialized").mockResolvedValue();
    ...
    it("fails clearly when a requested CDP target is not registered", async () => {
        ...
        await expect(manager.open("https://example.com")).rejects.toThrow("Target missing-target was not found in the attached browser.");
    });
    it("collects forwardable env vars from the caller's env", async () => {
        ...
        await expect(collectForwardedEnv({
    ...
    it("does not disturb an already-initialized session when forwarded env changes", () => {
        ...
        expect(manager.stagehand).not.toBeNull();
...
async function waitForSocket(socketPath, timeoutMs = 1_000) {
...
function sendSocketRequest(socketPath, request) {
    return new Promise((resolve, reject) => {
        ...
        socket.once("close", () => {
            if (!settled)
                finish(new Error("Socket closed before response."));
        });
    });
}
function delay(ms) {
