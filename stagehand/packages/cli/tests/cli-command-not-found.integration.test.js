...
describe("command_not_found hook (built CLI)", () => {
    it("prints a did-you-mean suggestion and preserves exit code 2", async () => {
        ...
        expect(result.stderr).toContain('"browse sessions" is not a browse command. Did you mean "browse cloud sessions list"? Run browse --help for all commands.');
    ...
    it("omits the did-you-mean clause when no decent match exists", async () => {
        ...
        expect(result.exitCode).toBe(2);
        expect(result.stderr).toContain('"browse frobnicate" is not a browse command. Run browse --help for all commands.');
    ...
    it("never sends raw argv values in not-found telemetry", async () => {
        ...
        try {
            ...
            const result = await runCli(["opne", "https://example.com/?token=supersecret"], { env: telemetryEnv(telemetryServer, installIdFile) });
            expect(result.exitCode).toBe(2);
            expect(result.stderr).toContain('Did you mean "browse open"?');
            const serialized = JSON.stringify(telemetryServer.requests.map((request) => request.jsonBody));
