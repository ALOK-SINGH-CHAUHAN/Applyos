...
import { binaryResponse, jsonResponse, startFakeBrowserbaseServer, textResponse, } from "./helpers/fake-browserbase-server.js";
...
describe("cloud API contracts", () => {
    ...
    it("fetch writes JSON content objects to an output file", async () => {
        ...
        await withServer(async (_request, response) => {
            ...
            expect(await readFile(outputPath, "utf8")).toBe(JSON.stringify({ title: "Example" }, null, 2));
        });
    });
    ...
    it("search writes results to an output file", async () => {
        ...
        await withServer(async (_request, response) => {
            jsonResponse(response, 200, makeSearchResponse("test", [
                { id: "res_1", url: "https://example.com", title: "Example" },
            ]));
        }, async ({ baseUrl }) => {
            const result = await runCli([
    ...
    it.each(["0", "false", "no", "FALSE", "NO"])("does not auto-load .env when BROWSE_LOAD_DOTENV=%s", async (optOutValue) => {
    ...
    ])("contexts contract: %j", async ({ args, expectedMethod, expectedPath, expectedBody, responseBody, }) => {
        await withServer(async (_request, response) => {
    ...
    ])("extensions get/delete contract: %j", async ({ args, expectedMethod, expectedPath, responseBody }) => {
        await withServer(async (_request, response) => {
            if (responseBody === null) {
                response.writeHead(204);
                response.end();
    ...
    ])("sessions contract: %j", async ({ args, expectedMethod, expectedPath, expectedBody, responseBody, }) => {
        await withServer(async (_request, response) => {
            jsonResponse(response, 200, responseBody);
        }, async ({ baseUrl, requests }) => {
            const result = await runCli([
    ...
    it("sessions list table renders missing keepAlive as unknown", async () => {
        await withServer(async (_request, response) => {
            ...
            expect(result.exitCode).toBe(0);
            expect(result.stdout).toMatch(/sess_111\s+COMPLETED\s+2026-05-17 07:45Z\s+2s\s+us-west-2\s+-\s+-\s+browse_cli,source=test/);
        });
    });
    ...
    it("sessions create tags userMetadata with browse_cli, cli_version, and install_id", async () => {
        ...
        await withServer(async (_request, response) => {
            ...
            expect(result.exitCode).toBe(0);
            expectRequest(requests[0], "POST", "/v1/sessions", "test-key");
            const body = requests[0]?.jsonBody;
...
function makeSearchResponse(query, results) {
    return {
        requestId: "req_123",
        query,
        results,
...
function makeSessionResponse(id, status = "COMPLETED") {
    return {
        createdAt: "2026-05-17T07:45:06.376Z",
...
function makeFetchResponse(content, contentType = "text/plain") {
    return {
        id: "fetch_123",
        content,
        contentType,
