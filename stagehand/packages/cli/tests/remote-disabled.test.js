...
import { autoSelectRemoteTarget, forwardedEnvKeys, remoteDoctorCheck, remoteStagehandOptions, resolveExplicitRemoteTarget, } from "../src/lib/driver/remote.disabled.js";
...
describe("remote.disabled (local-only capability)", () => {
    ...
    it("contains no BROWSERBASE_API_KEY reference in its source", async () => {
        ...
        const source = await readFile(fileURLToPath(new URL("../src/lib/driver/remote.disabled.ts", import.meta.url)), "utf8");
