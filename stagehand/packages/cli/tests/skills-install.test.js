...
import { quoteForCmdShell, shouldUseWindowsShell, spawnPassthrough, } from "../src/lib/skills/install.js";
...
describe("skills install", () => {
    ...
    itPosix("falls back to the GitHub installer when the catalog fetch hangs", async () => {
        ...
        expect(result.exitCode).toBe(0);
        await expect(readFile(logPath, "utf8")).resolves.toContain("--yes skills add browserbase/browse.sh --skill airline.example/book-flight-ab12cd");
    });
});
describe("quoteForCmdShell", () => {
    ...
    it("quotes install paths with spaces", () => {
        expect(quoteForCmdShell("C:\\Users\\First Last\\.config\\browserbase\\skill")).toBe('"C:\\Users\\First Last\\.config\\browserbase\\skill"');
    });
...
describe("spawnPassthrough", () => {
    it("kills a hung child after the deadline and reports a timeout", async () => {
        ...
        const result = await spawnPassthrough(process.execPath, ["-e", "setTimeout(() => {}, 600_000);"], 500);
