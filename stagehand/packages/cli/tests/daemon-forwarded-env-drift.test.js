 ...
 */
const FORWARDED_ENV = new Set(["BROWSERBASE_API_KEY"]);
/**
 ...
 */
const DAEMON_LOCAL_ENV = new Set([
    "BROWSE_DAEMON_DIR",
    "BROWSE_SESSION",
]);
/** Matches `process.env.NAME`, `process.env["NAME"]`, and `process.env['NAME']`. */
const ENV_READ = /process\.env(?:\.([A-Za-z_][A-Za-z0-9_]*)|\[\s*(["'])([A-Za-z_][A-Za-z0-9_]*)\2\s*\])/g;
...
async function collectEnvReads() {
...
describe("daemon forwarded-env drift guard", () => {
    ...
    it("categorizes every driver-path env read as forwarded or daemon-local", async () => {
        ...
        expect(uncategorized, uncategorized.length === 0
            ? undefined
            : [
