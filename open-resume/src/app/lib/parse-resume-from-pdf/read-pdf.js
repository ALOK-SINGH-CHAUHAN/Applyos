...
// @ts-ignore
const pdf_worker_entry_1 = __importDefault(require("pdfjs-dist/build/pdf.worker.entry"));
...
const readPdf = async (fileUrl) => {
    ...
    for (let i = 1; i <= pdfFile.numPages; i++) {
        ...
        const pageTextItems = textContent.items.map((item) => {
            const { str: text, dir, // Remove text direction
            transform, fontName: pdfFontName, ...otherProps } = item;
