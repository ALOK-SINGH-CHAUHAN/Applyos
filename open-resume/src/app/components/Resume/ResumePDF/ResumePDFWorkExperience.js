"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ResumePDFWorkExperience = void 0;
const renderer_1 = require("@react-pdf/renderer");
const common_1 = require("components/Resume/ResumePDF/common");
const styles_1 = require("components/Resume/ResumePDF/styles");
const ResumePDFWorkExperience = ({ heading, workExperiences, themeColor, }) => {
    return (<common_1.ResumePDFSection themeColor={themeColor} heading={heading}>
      {workExperiences.map(({ company, jobTitle, date, descriptions }, idx) => {
            // Hide company name if it is the same as the previous company
            const hideCompanyName = idx > 0 && company === workExperiences[idx - 1].company;
            return (<renderer_1.View key={idx} style={idx !== 0 ? { marginTop: styles_1.spacing["2"] } : {}}>
            {!hideCompanyName && (<common_1.ResumePDFText bold={true}>{company}</common_1.ResumePDFText>)}
            <renderer_1.View style={{
                    ...styles_1.styles.flexRowBetween,
                    marginTop: hideCompanyName
                        ? "-" + styles_1.spacing["1"]
                        : styles_1.spacing["1.5"],
                }}>
              <common_1.ResumePDFText>{jobTitle}</common_1.ResumePDFText>
              <common_1.ResumePDFText>{date}</common_1.ResumePDFText>
            </renderer_1.View>
            <renderer_1.View style={{ ...styles_1.styles.flexCol, marginTop: styles_1.spacing["1.5"] }}>
              <common_1.ResumePDFBulletList items={descriptions}/>
            </renderer_1.View>
          </renderer_1.View>);
        })}
    </common_1.ResumePDFSection>);
};
exports.ResumePDFWorkExperience = ResumePDFWorkExperience;
