import { StructuredResume } from '@autoapply/shared';

export function renderResumeToHtml(resume: StructuredResume): string {
...
<body>
  ...
  ${experience
    .map(
      (exp: { title: string; company: string; startDate: string; endDate?: string; current?: boolean; highlights: string[] }) => `
    <div>
  ...
  ${education
    .map(
      (edu: { degree: string; institution: string; graduationYear?: string }) => `
    <div>
