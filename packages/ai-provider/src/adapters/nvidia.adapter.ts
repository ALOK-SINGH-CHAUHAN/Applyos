...
export class NvidiaProvider implements AIProvider {
  ...
  constructor(private readonly apiKey: string = (process.env.NVIDIA_API_KEY || '').replace(/['"]/g, '')) {}

  async generateText(input: GenerateTextInput): Promise<GenerateTextOutput> {
    if (!this.apiKey || this.apiKey.includes('mock-')) {
      throw new Error('NVIDIA_API_KEY is not configured or is set to a mock key. Real AI is required.');
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`NVIDIA API call failed: ${response.status} - ${errorText}`);
      }
