...
export class GroqProvider implements AIProvider {
  ...
  constructor(private readonly apiKey: string = (process.env.GROQ_API_KEY || '').replace(/['"]/g, '')) {}

  async generateText(input: GenerateTextInput): Promise<GenerateTextOutput> {
    if (!this.apiKey || this.apiKey.includes('mock-')) {
      throw new Error('GROQ_API_KEY is not configured or is set to a mock key. Real AI is required.');
    }
    ...
    try {
      ...
      // Estimate Cost: input: $0.05 / 1M, output: $0.08 / 1M
